...
// VERSION: v7.3.6 (SW-Release V7.3)
// DATUM: 07. Januar 2026
// BESCHREIBUNG: Zeiterfassung - Monatsansicht pro Mitarbeiter
// BASIERT AUF: Excel-Stundennachweis + V6 Zeiterfassung
...
// Zeiterfassungs-Daten pro Tag
interface DayEntry {
  hours: number | null;
  absenceCode: string | null; // U, K, F, S
}
...
export default function ZeiterfassungPage() {
  ...
  const [nonBillableHours, setNonBillableHours] = useState<Record<number, number>>({}); // Tag -> Stunden
  const [absences, setAbsences] = useState<Record<number, string>>({}); // Tag -> Code (U/K/S)
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      // Wenn admin: ersten MA auswählen, sonst eigenen
      if (profile.role === 'client_admin' && employeesData && employeesData.length > 0) {
        setSelectedEmployeeId(employeesData[0].id);
      } else {
        // Eigenen Employee finden (über email)
        const ownEmployee = employeesData?.find(e => 
          e.display_name?.toLowerCase().includes(profile.display_name?.toLowerCase() || '')
        );
        if (ownEmployee) {
  ...
  // ============================================
  // ZEITERFASSUNGS-ZEILEN AUFBAUEN
  // ============================================
  ...
  useEffect(() => {
    ...
    workPackages.forEach(wp => {
      // Nur APs anzeigen, wo MA zugeordnet ist ODER zum Projekt gehört
    ...
    // TODO: Bestehende Zeiterfassungs-Daten aus DB laden
    loadExistingTimeEntries(selectedEmployeeId, selectedYear, selectedMonth);

  }, [selectedEmployeeId, projects, workPackages, projectAssignments, wpAssignments, selectedYear, selectedMonth]);

  // ============================================
  // BESTEHENDE ZEITERFASSUNG LADEN
  // ============================================

  const loadExistingTimeEntries = async (employeeId: string, year: number, month: number) => {
    // TODO: Implementieren wenn DB-Schema angepasst
    // Hier würden wir v7_timesheets laden
    console.log('TODO: Lade Zeiterfassung für', employeeId, year, month);
  };
  ...
  const getDayOfWeek = (year: number, month: number, day: number): number => {
    return new Date(year, month - 1, day).getDay(); // 0 = Sonntag
  };
  ...
  const handleKeyDown = (
    ...
    switch (e.key) {
      ...
      case 'Enter':
        e.preventDefault();
        // Nächste Zelle rechts oder nächste Zeile
        if (day < daysInMonth) {
          nextDay = day + 1;
  ...
  const handleSave = async () => {
    ...
    try {
      // TODO: In v7_timesheets speichern
      // Hier müssen wir die Daten in die DB schreiben
      
      console.log('Speichere Zeiterfassung:', {
      ...
      setSuccessMessage('Zeiterfassung gespeichert!');
  ...
  const getTotalNonBillableHours = (): number => {
    return Object.values(nonBillableHours).reduce((sum, h) => sum + h, 0);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b shadow-sm">
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            {/* Mitarbeiter-Auswahl (nur für Admin) */}
            <div className="flex items-center gap-4">
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 mt-4">
        <div className="bg-white rounded-lg shadow p-3 flex flex-wrap items-center gap-4 text-sm">
          ...
          {Object.entries(ABSENCE_CODES).map(([code, { label, color }]) => (
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="bg-white rounded-lg shadow overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {/* 1. Förderbare Arbeiten (Arbeitspakete) */}
              ...
              <tr className="hover:bg-gray-50">
                ...
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  const weekend = isWeekend(selectedYear, selectedMonth, day);
                  const holiday = isHoliday(selectedYear, selectedMonth, day);
                  const currentAbsence = absences[day] || '';
                  
                  // An Feiertagen automatisch F anzeigen
                  if (holiday) {
                    return (
