...
// VERSION: v7.0.3 - pypdf-basierte XFA-Extraktion
// DATUM: 30. Dezember 2024
// WICHTIG: Erfordert Python mit pypdf auf dem Server!
...
// ============================================
// HELPER: XFA aus PDF extrahieren (Buffer-basiert)
// ============================================

async function extractXfaFromPdf(buffer: Buffer): Promise<string | null> {
  // Konvertiere Buffer zu String (latin-1 für binäre PDF-Daten)
  const pdfText = buffer.toString('latin1')
  
  // Methode 1: Suche nach unkomprimierten XFA-Daten
  if (pdfText.includes('<xfa:data') || pdfText.includes('<xfa:datasets')) {
    // XFA-Daten sind unkomprimiert vorhanden
    const startIdx = pdfText.indexOf('<xfa:datasets')
    if (startIdx === -1) {
      const altStart = pdfText.indexOf('<xfa:data')
      if (altStart !== -1) {
        const endIdx = pdfText.indexOf('</xfa:data>', altStart)
        if (endIdx !== -1) {
          return pdfText.substring(altStart, endIdx + 11)
        }
      }
      return null
    }
    const endIdx = pdfText.indexOf('</xfa:datasets>', startIdx)
    if (endIdx !== -1) {
      return pdfText.substring(startIdx, endIdx + 15)
    }
  }
  
  // Methode 2: Suche nach datasets-Marker (auch mit Zeilenumbrüchen)
  const datasetsMatch = pdfText.match(/datasets[\s\S]*?<xfa:data[\s\S]*?<\/xfa:data[^>]*>/i)
  if (datasetsMatch) {
    return datasetsMatch[0]
  }
...
// Berücksichtigt Zeilenumbrüche in Tags
// ============================================
...
// Flexibles Pattern das Zeilenumbrüche in Tags erlaubt
function flexPattern(tagName: string): RegExp {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // PDF-Inhalt als Text extrahieren (latin-1 für binäre Daten)
    let pdfText = buffer.toString('latin1')
    
    // Zeilenumbrüche in Tags normalisieren für einfacheres Parsing
    // <tag\n> -> <tag>
    pdfText = pdfText.replace(/<([^>]+)\n>/g, '<$1>')
    
    // XFA-Daten prüfen
    const hasXfa = pdfText.includes('xfa:data') || 
                   pdfText.includes('datasets') ||
                   pdfText.includes('cg_VMS_') ||
                   pdfText.includes('Seite2_AST')
    ...
    const projekt: ZimProjekt = {
      ...
      gesamt_pk: extractFloat(/<sum_ges_pk>([^<]+)/, pdfText) ||
                 extractFloat(/<cg_VMS_HB_A_Kosten>([^<]+)/, pdfText),
      laufzeit_monate: 0, // Wird unten berechnet
    }
    ...
    const budget: ZimBudget = {
      ...
      materialkosten: extractFloat(/<cg_VMS_HB_A_Material>([^<]+)/, pdfText) ||
                      extractFloat(/<cg_VMS_HB_A_Jahr1Kost02>([^<]+)/, pdfText),
      fremdleistungen: extractFloat(/<cg_VMS_HB_A_Fremdleist>([^<]+)/, pdfText) ||
                       extractFloat(/<cg_VMS_HB_A_Jahr1Kost03>([^<]+)/, pdfText),
    ...
    // Versuche verschiedene Patterns für Anlage 6.2
    const a62Patterns = [
      /<cg_file_262_Zeile1_Anlage62>([\s\S]*?)<\/cg_file_262_Zeile1_Anlage62>/g,
      /<cg_file_262[^>]*>([\s\S]*?)<\/cg_file_262[^>]*>/g,
    ]

    for (const pattern of a62Patterns) {
      let a62Match
      while ((a62Match = pattern.exec(pdfText)) !== null) {
        const block = a62Match[1]
        const maId = extractValue(/<cg_VMS_PK_DdsId_261>([^<]+)/, block) ||
                     extractValue(/<lfd>([^<]+)/, block)
        if (maId) {
          const pmProJahr: Record<number, number> = {}
          
          // PM pro Jahr extrahieren
    ...
    for (const pattern of maPatterns) {
      ...
      while ((maMatch = pattern.exec(pdfText)) !== null) {
        ...
        mitarbeiter.push({
          ...
          jahresbrutto: extractFloat(/<Jahresbrutto>([^<]+)/, block) ||
                        extractFloat(/<cg_VMS_PM_iJahresbrutto>([^<]+)/, block),
      ...
      // Wenn Mitarbeiter gefunden, nicht weitere Patterns probieren
    ...
    // Methode 1: Arbeitsplan-Zeilen (klassisches Format)
    ...
    // Methode 2: Direkte AP-Namen (falls Methode 1 nichts findet)
    if (apTempMap.size === 0) {
