...
**Status:** V7.4.3-19 deployed auf v7-dev + main - Personalkosten Excel-Export mit Zeitraum-Filter
...
| /v7/berater/foerderung/firma/[id]/berichte | 7.4.3-12 | Berichte und Controlling mit Stundennachweis-Matrix |
...
### 7.27 ZA-Modul Konzept (v7.4.4 - geplant)
...
Erforderliche DB-Erweiterungen (noch nicht deployed):
  -- In v7_projects:
  ALTER TABLE v7_projects
    ADD COLUMN foerdersatz     NUMERIC(5,2),  -- z.B. 45.00 (%)
    ADD COLUMN overhead_t      NUMERIC(6,2),  -- z.B. 28.42 (%)
    ADD COLUMN overhead_nt     NUMERIC(6,2),  -- z.B. 29.88 (%)
    ADD COLUMN overhead_gleich BOOLEAN DEFAULT false;

  -- Neue Tabelle:
  CREATE TABLE v7_zahlungsanforderungen (
    id                     UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id             UUID NOT NULL REFERENCES v7_projects(id),
    za_nummer              INTEGER NOT NULL,         -- 1, 2, 3... (editierbar)
...
Geplante ZA-Kachel UI (3-Tab-Dialog):
- Tab "Deckblatt" (Seite 5): Alle Kostenarten, Gemeinkostenzuschlag,
  Foerdersatz, berechnete anteilige Zuwendung - alle Felder editierbar
- Tab "Anlage 1a" (Seite 6): Tabelle MA x Monat mit Stunden aus Zeiterfassung
  (Typ DS: zwei Spalten T/NT)
- Tab "Anlage 1b" (Seite 7): MA, foerderbare Stunden, Stundensatz,
  zuwendungsfaehige Personalkosten (Typ DS: T/NT getrennt)

ZA-Nummer: Seriell vorbelegt (naechste freie Nummer), editierbar.
Formulartyp-Erkennung: automatisch aus project.funding_format.
...
### 9.1 Kurzfristig (v7.4.4 - naechste Session)
...
- **ZA-Modul (Kachel 4) - Naechste Prioritaet:**
  1. SQL-Migration: foerdersatz/overhead_t/overhead_nt in v7_projects +
     neue Tabelle v7_zahlungsanforderungen (Script fertig in Abschnitt 7.27)
  2. ProjectDetailPage/-CreateForm: Fördersatz + Overhead-Felder ergaenzen
  3. ZA-Kachel: 3-Tab-Dialog (Deckblatt / Anlage 1a / Anlage 1b)
     Formulartyp-abhaengig (ZIM vs ZIM_DS) - Details in Abschnitt 7.27
...
|---------|-------|-------------|
| v4.40 | 09.03.2026 | v7.4.3-19: Personalkosten Excel-Export (Zeitraum-Filter, xlsx, 2 Sheets), ZA-Modul Konzept (Abschnitt 7.26+7.27) |
...
**Ende des Pflichtenhefts v4.40**
**Letzte Aktualisierung: 09. Maerz 2026**
