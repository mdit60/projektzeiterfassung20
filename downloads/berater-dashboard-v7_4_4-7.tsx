...
// Datum: 17. April 2026
//
// v7.4.4-7: NEU: Offene Rueckfragen-Zaehler in Kunden-Kachel
//   - Zeigt Anzahl offener v7_timesheet_notes
//   - Orange AlertCircle wenn > 0, gruen CheckCircle wenn alle erledigt
        ...
        if (profile) {
          ...
          if (profile.consultant_company_id) {
            ...
            if (companyIds.length > 0) {
              ...
              // NEU v7.4.4-7: Offene Timesheet-Notizen zaehlen
              const allProjectIds = (await supabase
                .from('v7_projects')
                .select('id')
                .in('client_company_id', companyIds)
                .eq('is_active', true)
              ...
              if (allProjectIds.length > 0) {
                const { count: notizenCount } = await supabase
                  .from('v7_timesheet_notes')
                  .select('*', { count: 'exact', head: true })
                  .in('project_id', allProjectIds)
                  .eq('status', 'offen');
                offeneNotizen = notizenCount || 0;
              }
            }
