...
//
// Enthaelt ALLE ZA-Logik:
//   - Typen: ZahlungsanforderungDB, ZAFormData, ProjectAssignment, etc.
//   - Datenladung: project_assignments (mit v7_employees JOIN)
//   - Berechnungen: getZAPersonenstunden, getHourlyRate, Deckblatt-Werte
//   - UI: Tabs Deckblatt / Anlage 1a / Anlage 1b, Drucken, Speichern
//
// Props:
//   portal        - 'berater' | 'firma'  -> Farbschema
//   projects      - Projektliste der Firma (bereits geladen)
//   workPackages  - Arbeitspakete (bereits geladen)
//   wpAssignments - WP-Zuordnungen (bereits geladen)
//   employees     - Mitarbeiterliste (bereits geladen)
//   timesheets    - Zeiterfassungen (bereits geladen, is_active=true gefiltert)
//   projectAssignments - aus v7_project_assignments inkl. v7_employees JOIN
//   initialProjectId   - optional: Projekt vorauswaehlen
// ============================================================================
...
// ============================================================================
// HILFS-FUNKTION: project_assignments mit v7_employees JOIN laden
// Wird von den Page-Komponenten aufgerufen, um projectAssignments zu befuellen
// ============================================================================

export async function loadProjectAssignments(
  projectIds: string[]
): Promise<ZAProjectAssignment[]> {
  ...
  if (error) {
    console.error('ZAPanel: project_assignments Fehler:', error);
    return [];
  }
...
const PORTAL_COLORS = {
  berater: {
    ...
    btnZaSelected: 'bg-blue-600 text-white border-blue-600',
    btnZaHover: 'hover:border-blue-400',
    btnNeueZA: 'border-dashed border-blue-400 text-blue-600 hover:bg-blue-50',
    focusRing: 'focus:ring-blue-500',
    inputFocus: 'focus:ring-1 focus:ring-blue-500',
    kachelBorder: 'border-blue-200',
    kachelBorderActive: 'border-blue-600',
    kachelBg: 'bg-white',
    kachelBgActive: 'bg-blue-50',
    kachelText: 'text-blue-700',
    kachelChevronBg: 'bg-blue-100',
    kachelChevronBgActive: 'bg-blue-200',
  ...
  firma: {
    ...
    btnZaSelected: 'bg-green-600 text-white border-green-600',
    btnZaHover: 'hover:border-green-400',
    btnNeueZA: 'border-dashed border-green-400 text-green-600 hover:bg-green-50',
    focusRing: 'focus:ring-green-500',
    inputFocus: 'focus:ring-1 focus:ring-green-500',
    kachelBorder: 'border-green-200',
    kachelBorderActive: 'border-green-600',
    kachelBg: 'bg-white',
    kachelBgActive: 'bg-green-50',
    kachelText: 'text-green-700',
    kachelChevronBg: 'bg-green-100',
    kachelChevronBgActive: 'bg-green-200',
...
interface ZAPanelProps {
  ...
  // Kontrollierter State fuer Button/Panel-Split (beide Instanzen teilen denselben State)
...
export default function ZAPanel({
  ...
  // ZIM-relevante Projekte (ZIM oder ZIM_DS)
  const zimProjects = projects.filter(p =>
    p.funding_format === 'ZIM' || p.funding_format === 'ZIM_DS'
  );

  // ---- State ----
  const [showPanelInternal, setShowPanelInternal] = useState(false);
  // Wenn controlledShow uebergeben: externer State; sonst interner State
  const showPanel = controlledShow !== undefined ? controlledShow : showPanelInternal;
  const togglePanel = onToggle !== undefined ? onToggle : () => setShowPanelInternal(prev => !prev);
  ...
  // ---- Kein ZIM-Projekt verfuegbar ----
  if (zimProjects.length === 0) {
    return null;
  }

  // ---- Berechnungen fuer aktuelles Panel ----
  ...
  const kachelButton = (
    <button
      ...
      onMouseLeave={e => { if (!showPanel) (e.currentTarget as HTMLButtonElement).style.backgroundColor = ''; }}
  ...
  return (
    <div id="za-kachel">
      ...
      {/* Panel */}
      {showPanel && (
        <div className={`mt-6 border ${colors.border} rounded-lg overflow-hidden`}>
