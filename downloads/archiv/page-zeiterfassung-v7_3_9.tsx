...
// VERSION: v7.3.9 (SW-Release V7.3)
// DATUM: 08. Januar 2026
// BESCHREIBUNG: Zeiterfassung - Excel-konformes Layout mit PDF-Export
// ÄNDERUNG: 
//   - Druck auf eine Seite optimiert (kleinere Schrift, Landscape A4)
//   - "Zeitausgleich" → "sonstige Arbeiten"
//   - Spalte V/T: standardmäßig ausgeblendet, für Durchführbarkeitsstudien (DS) als T-Spalte einblendbar
...
interface Project {
  ...
  funding_program: string | null; // ZIM-EP, ZIM-KF, ZIM-DS, etc.
}
...
interface WorkPackage {
  ...
  ap_code: string | null;
  name: string;
  is_technical: boolean | null; // Für DS: T=technisch, leer=nicht-technisch
}
...
interface APRow {
  ...
  isTechnical: boolean; // Für DS: technisches AP?
}
...
export default function ZeiterfassungPage() {
  ...
  const [apRows, setApRows] = useState<APRow[]>([
    { workPackageId: null, entries: {}, isTechnical: true },
    { workPackageId: null, entries: {}, isTechnical: true },
    { workPackageId: null, entries: {}, isTechnical: true },
    { workPackageId: null, entries: {}, isTechnical: true },
  ]);

  // Nicht zuschussfähige Arbeiten (eine Zeile) - vormals "Zeitausgleich"
  ...
  // Prüfen ob Durchführbarkeitsstudie (DS)
  const isDurchfuehrbarkeitsstudie = selectedProject?.funding_program?.includes('DS') || false;
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      // Projekte laden (mit funding_program für DS-Erkennung)
      const { data: projectsData } = await supabase
        .from('v7_projects')
        .select('id, name, short_name, funding_reference, funding_program')
        .eq('client_company_id', companyId)
        .eq('is_active', true);
      ...
      if (projectsData && projectsData.length > 0) {
        ...
        // Arbeitspakete laden (mit is_technical für DS)
    ...
    const loadTimeEntries = async () => {
      ...
      entries?.forEach(entry => {
        ...
        if (entry.work_package_id && wpIds.includes(entry.work_package_id)) {
          ...
          // Nicht zuschussfähig (sonstige Arbeiten)
      ...
      wpEntryMap.forEach((dayMap, wpId) => {
        if (rowIndex < 4) {
          ...
          // is_technical vom Work Package holen
          const wp = projectWPs.find(w => w.id === wpId);
          newRows[rowIndex] = { 
            workPackageId: wpId, 
            entries: entriesObj, 
            isTechnical: wp?.is_technical ?? true 
          };
          rowIndex++;
        }
      });
  ...
  const handleAPSelect = (rowIndex: number, wpId: string) => {
    // is_technical vom ausgewählten Work Package übernehmen
    const wp = workPackages.find(w => w.id === wpId);
    setApRows(prev => {
      const updated = [...prev];
      updated[rowIndex] = { 
        ...updated[rowIndex], 
        workPackageId: wpId || null,
        isTechnical: wp?.is_technical ?? true
      };
      return updated;
    });
  ...
  const handleTechnicalToggle = (rowIndex: number, isTechnical: boolean) => {
    setApRows(prev => {
      const updated = [...prev];
      updated[rowIndex] = { ...updated[rowIndex], isTechnical };
      return updated;
    });
    setHasChanges(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      apRows.forEach(row => {
        ...
        Object.entries(row.entries).forEach(([dayStr, entry]) => {
          ...
          const record = {
            ...
            is_technical: row.isTechnical, // Für DS speichern
      ...
      // Nicht zuschussfähige Stunden (sonstige Arbeiten)
    ...
    <div className="min-h-screen bg-gray-100">
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-3">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-bold text-white">Stundennachweis</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handlePrint}
                className="flex items-center gap-1 px-3 py-1.5 bg-white text-gray-700 rounded text-sm font-medium hover:bg-gray-50"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              {projects.map(p => (
                <option key={p.id} value={p.id}>
                  ...
                  {p.funding_program?.includes('DS') ? ' [DS]' : ''}
                </option>
              ))}
            </select>
          </div>
      ...
      <div ref={printRef} className="max-w-full mx-auto p-4 print:p-0 print:m-0">
        <div className="bg-white shadow-lg print:shadow-none overflow-x-auto">
          {/* Header-Bereich */}
          <table className="w-full border-collapse text-xs print:text-[7pt]" style={{ minWidth: '1000px' }}>
            <tbody>
              {/* Zeile 1: Titel rechts */}
              <tr>
                <td className="border p-1 print:p-0.5" style={{ width: '50%' }} rowSpan={2}>
                  <div className="text-[10px] print:text-[6pt] text-gray-500">Zuwendungsempfänger (Firmenstempel)</div>
                  <div className="font-semibold mt-1 print:mt-0">{company?.name}</div>
                </td>
                <td className="border p-1 print:p-0.5 text-right" style={{ backgroundColor: COLORS.headerOrange }}>
                  <span className="text-[10px] print:text-[6pt]">Förderkennzeichen:</span>
                </td>
                <td className="border p-1 print:p-0.5 font-bold" style={{ backgroundColor: COLORS.headerOrange }}>
                  {selectedProject?.funding_reference || '-'}
                </td>
              </tr>
              <tr>
                <td className="border p-1 print:p-0.5 text-center font-bold text-lg print:text-sm" colSpan={2} style={{ backgroundColor: COLORS.headerOrange }}>
                  Stundennachweis
                </td>
              </tr>
              <tr>
                <td className="border p-1 print:p-0.5" colSpan={3}>
                  <span className="text-[10px] print:text-[6pt] text-gray-500">Der Stundennachweis verbleibt beim Zuwendungsempfänger und ist nur nach Aufforderung vorzulegen.</span>
                </td>
              </tr>
              ...
              <tr>
                <td className="border p-1 print:p-0.5" colSpan={3}>
                  <span className="text-[10px] print:text-[6pt] text-gray-500">Vorhabenthema</span>
              ...
              <tr>
                <td className="border p-1 print:p-0.5">
                  <span className="text-[10px] print:text-[6pt] text-gray-500">Monat</span>
                  <div className="font-semibold">{formatDisplayDate()}</div>
                </td>
                <td className="border p-1 print:p-0.5" colSpan={2}>
                  <span className="text-[10px] print:text-[6pt] text-gray-500">Mitarbeiter(in): [Name, Vorname]</span>
          ...
          {/* Hinweistext */}
          <div className="p-2 print:p-1 text-[9px] print:text-[6pt] text-gray-600 border-x">
          ...
          <table className="w-full border-collapse text-xs print:text-[7pt]" style={{ minWidth: '1000px' }}>
            <thead>
              <tr style={{ backgroundColor: COLORS.headerOrange }}>
                <th className="border p-1 print:p-0.5 text-left" style={{ width: '30px' }}>lfd. Nr.</th>
                <th className="border p-1 print:p-0.5 text-left" style={{ width: '30px' }}>AP</th>
                <th className="border p-1 print:p-0.5 text-left" style={{ width: '150px', maxWidth: '150px' }}>Kurzbezeichnung des Arbeitspakets</th>
                {/* T-Spalte nur für Durchführbarkeitsstudien */}
                {isDurchfuehrbarkeitsstudie && (
                  <th className="border p-1 print:p-0.5 text-center" style={{ width: '20px' }} title="T = Technisches AP">T</th>
                )}
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  ...
                  return (
                    <th
                      key={day}
                      className={`border p-0 print:p-0 text-center ${weekend ? 'bg-gray-300' : holiday ? 'bg-orange-200' : ''}`}
                ...
                <th className="border p-1 print:p-0.5 text-center" style={{ width: '45px' }}>Summe<br/>Monat</th>
              </tr>
            </thead>
            <tbody>
              {/* Abschnitt 1: Förderbare Arbeiten */}
              <tr>
                <td className="border p-1 print:p-0.5 font-semibold" colSpan={(isDurchfuehrbarkeitsstudie ? 4 : 3) + daysInMonth + 1} style={{ backgroundColor: '#FFF9E6' }}>
              ...
              {apRows.map((row, rowIndex) => {
                ...
                return (
                  <tr key={rowIndex}>
                    <td className="border p-1 print:p-0.5 text-center">{rowIndex + 1}.</td>
                    <td className="border p-0">
                      <select
                        value={row.workPackageId || ''}
                        onChange={(e) => handleAPSelect(rowIndex, e.target.value)}
                        className="w-full h-full p-1 text-xs print:text-[7pt] border-0 bg-transparent print:appearance-none"
                      >
                        <option value="">-</option>
                    ...
                    <td className="border p-1 print:p-0.5 text-[10px] print:text-[6pt] leading-tight" style={{ maxWidth: '150px' }}>
                      <div className="line-clamp-2 print:line-clamp-1" title={selectedWP?.name}>
                        {selectedWP?.name || ''}
                      </div>
                    </td>
                    {/* T-Spalte für DS */}
                    {isDurchfuehrbarkeitsstudie && (
                      <td className="border p-0 text-center">
                        {row.workPackageId ? (
                          <input
                            type="checkbox"
                            checked={row.isTechnical}
                            onChange={(e) => handleTechnicalToggle(rowIndex, e.target.checked)}
                            className="w-4 h-4 print:w-3 print:h-3"
                            title={row.isTechnical ? 'Technisches AP' : 'Nicht-technisches AP'}
                          />
                        ) : (
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            maxLength={4}
                            className={`w-full h-7 print:h-5 text-center text-xs print:text-[7pt] border-0 ${
                    ...
                    <td className="border p-1 print:p-0.5 text-center font-semibold bg-gray-50">
              ...
              <tr className="font-semibold" style={{ backgroundColor: '#E8F5E9' }}>
                <td className="border p-1 print:p-0.5" colSpan={isDurchfuehrbarkeitsstudie ? 4 : 3}>Summe der förderbaren Stunden (2)</td>
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  const daySum = calculateDaySum(day);
                  return (
                    <td key={day} className="border p-1 print:p-0.5 text-center text-[10px] print:text-[6pt]">
                      {daySum > 0 ? daySum.toFixed(2) : '0,00'}
                    </td>
                  );
                })}
                <td className="border p-1 print:p-0.5 text-center bg-green-200">
              ...
              <tr>
                <td className="border p-1 print:p-0.5 font-semibold" colSpan={(isDurchfuehrbarkeitsstudie ? 4 : 3) + daysInMonth + 1} style={{ backgroundColor: '#FFF3E0' }}>
                  2. Nicht zuschussfähige Arbeiten
                </td>
              </tr>
              <tr>
                <td className="border p-1 print:p-0.5" colSpan={isDurchfuehrbarkeitsstudie ? 4 : 3}>sonstige Arbeiten</td>
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  const weekend = isWeekend(selectedYear, selectedMonth, day);
                  const holiday = isHoliday(selectedYear, selectedMonth, day);
                  ...
                  return (
                    <td key={day} className={`border p-0 text-center ${weekend ? 'bg-gray-200' : holiday ? 'bg-orange-100' : ''}`}>
                      <input
                        ...
                        maxLength={4}
                        className={`w-full h-5 print:h-4 text-center text-xs print:text-[7pt] border-0 ${
                ...
                <td className="border p-1 print:p-0.5 text-center font-semibold bg-yellow-50">
              ...
              <tr>
                <td className="border p-1 print:p-0.5 font-semibold" colSpan={(isDurchfuehrbarkeitsstudie ? 4 : 3) + daysInMonth + 1} style={{ backgroundColor: '#E3F2FD' }}>
                  3. Fehlzeiten
                </td>
              </tr>
              {/* Urlaub */}
              <tr>
                <td className="border p-1 print:p-0.5 text-[10px] print:text-[6pt]" colSpan={isDurchfuehrbarkeitsstudie ? 4 : 3}>Urlaub (nur bezahlten Urlaub aufführen)</td>
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  const absences = getAbsencesForDay(day);
                  const hasU = absences.some(a => a.code === 'U');
                  return (
                    <td key={day} className="border p-1 print:p-0.5 text-center text-[10px] print:text-[6pt] bg-blue-50">
                      {hasU ? DAILY_HOURS.toFixed(0) : ''}
                    </td>
                  );
                })}
                <td className="border p-1 print:p-0.5 text-center font-semibold bg-blue-100">
                  {absenceSums.U > 0 ? absenceSums.U.toFixed(2) : '0,00'}
                </td>
              </tr>
              {/* Krankheit */}
              <tr>
                <td className="border p-1 print:p-0.5 text-[10px] print:text-[6pt]" colSpan={isDurchfuehrbarkeitsstudie ? 4 : 3}>Krankheit (nur bei Lohn- und Gehaltsfortzahlung)</td>
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  const absences = getAbsencesForDay(day);
                  const hasK = absences.some(a => a.code === 'K');
                  return (
                    <td key={day} className="border p-1 print:p-0.5 text-center text-[10px] print:text-[6pt] bg-red-50">
                      {hasK ? DAILY_HOURS.toFixed(0) : ''}
                    </td>
                  );
                })}
                <td className="border p-1 print:p-0.5 text-center font-semibold bg-red-100">
                  {absenceSums.K > 0 ? absenceSums.K.toFixed(2) : '0,00'}
                </td>
              </tr>
              {/* Sonstige */}
              <tr>
                <td className="border p-1 print:p-0.5 text-[10px] print:text-[6pt]" colSpan={isDurchfuehrbarkeitsstudie ? 4 : 3}>Sonstige bezahlte Ausfallzeiten (z. B. Feiertage)</td>
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
                  const absences = getAbsencesForDay(day);
                  const hasS = absences.some(a => a.code === 'S');
                  const holiday = isHoliday(selectedYear, selectedMonth, day);
                  return (
                    <td key={day} className={`border p-1 print:p-0.5 text-center text-[10px] print:text-[6pt] ${holiday ? 'bg-orange-100' : 'bg-purple-50'}`}>
                      {hasS || holiday ? DAILY_HOURS.toFixed(0) : ''}
                    </td>
                  );
                })}
                <td className="border p-1 print:p-0.5 text-center font-semibold bg-purple-100">
          ...
          {/* Hinweistexte */}
          <div className="p-2 print:p-1 text-[8px] print:text-[5pt] text-gray-600 border-x border-b space-y-1 print:space-y-0">
            <p>
          ...
          {/* Unterschriften */}
          <div className="grid grid-cols-2 gap-4 print:gap-2 p-4 print:p-2 border-x border-b">
            <div className="border-t-2 border-gray-400 pt-2 print:pt-1">
              <div className="text-[10px] print:text-[6pt] text-gray-500 mb-6 print:mb-3">Datum / Unterschrift des Mitarbeiters</div>
              <div className="text-sm print:text-[8pt]">{getTodayFormatted()}</div>
            </div>
            <div className="border-t-2 border-gray-400 pt-2 print:pt-1">
              <div className="text-[10px] print:text-[6pt] text-gray-500 mb-6 print:mb-3">Datum / Unterschrift Geschäftsführer bzw. FuE-Verantwortlicher (in öffentlichen Forschungseinrichtungen)</div>
              <div className="text-sm print:text-[8pt]">{getTodayFormatted()}</div>
            </div>
          </div>
        </div>
      ...
      {/* Print Styles - optimiert für eine A4-Seite Landscape */}
      <style jsx global>{`
        @media print {
          html, body {
          ...
          /* Sicherstellen, dass alles auf eine Seite passt */
          .bg-white {
