...
//
// FIX: Kurzname wird aus Projekttitel extrahiert (Teil vor dem Doppelpunkt)
//      z.B. "BioInk: Analyse und Bewertung..." -> kurzname = "BioInk"
//
// Unterstuetzt:
// - ZIM Einzelprojekt-Antraege
// - ZIM Kooperationsprojekt-Antraege  
// - ZIM Durchfuehrbarkeitsstudien (Antrag_DS)
//
// Verwendet pdf-parse fuer robuste PDF-Extraktion
// ============================================================================
...
function parseZimData(text: string): any {
  ...
  if (isDurchfuehrbarkeitsstudie) {
    ...
    // NICHT kurzfass als kurzname verwenden - das ist die Beschreibung!
    // Kurzname wird unten aus dem Projektnamen extrahiert (Teil vor dem Doppelpunkt)
  ...
  if (isStandardZim || !projekt.name) {
    ...
    projekt.start = extractValue(normalized,
      /<cg_VMS_VB_Beginn>([^<]+)</,
      /<Beginn>([^<]+)</,
      /<start>([^<]+)</i
    )
    projekt.ende = extractValue(normalized,
      /<cg_VMS_VB_Ende>([^<]+)</,
      /<Ende>([^<]+)</,
      /<ende>([^<]+)</i
    )
  ...
  // ========================================
  // KURZNAME: Aus Projekttitel extrahieren wenn nicht vorhanden
  // "BioInk: Analyse und Bewertung..." -> "BioInk"
  // ========================================
  if (!projekt.kurzname && projekt.name) {
    if (projekt.name.includes(':')) {
      // Teil vor dem Doppelpunkt als Kurzname
      projekt.kurzname = projekt.name.split(':')[0].trim()
    } else if (projekt.name.includes(' - ')) {
      // Alternativ: Teil vor " - " als Kurzname
      projekt.kurzname = projekt.name.split(' - ')[0].trim()
    } else {
      // Fallback: Ersten 30 Zeichen oder erstes Wort
      const firstWord = projekt.name.split(' ')[0]
      if (firstWord.length >= 3 && firstWord.length <= 20) {
        projekt.kurzname = firstWord
      }
    }
  }
