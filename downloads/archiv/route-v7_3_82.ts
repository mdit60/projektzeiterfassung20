    ...
    if (result.success && result.data) {
      ...
      return NextResponse.json({
        ...
        arbeitspakete: (data.arbeitspakete || []).map((ap: any) => ({
          ap_number: ap.ap_number || ap.ap_nr || 0,
          ap_sub_number: ap.ap_sub_number || ap.ap_sub_nr || null,
          ap_code: ap.ap_code || `AP${ap.ap_number || ap.ap_nr}`,
          name: ap.name || ap.bezeichnung || '',
          start_date: ap.start_date || ap.von || null,
          end_date: ap.end_date || ap.bis || null,
          total_person_months: ap.total_person_months ?? ap.gesamt_pm ?? null,
          is_technical: ap.is_technical ?? false,
          mitarbeiter_zuordnungen: ap.mitarbeiter_zuordnungen || ap.zuordnungen || []
        })),
