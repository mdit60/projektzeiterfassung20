...
export default function WorkPackageList({
  ...
  // ============================================
  // RENDER: Einzelne Tabellenzeile
  // ============================================
  
  const renderWorkPackageRow = (wp: WorkPackage, isLast: boolean) => {
    ...
    return (
      <div
        key={wp.id}
        className="p-4 bg-white hover:bg-gray-50 transition-colors border-b border-gray-300"
      >
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* PM und Stunden */}
            <div className="text-right pl-3 border-l border-gray-300 min-w-[80px]">
    ...
    <div className={className}>
      ...
      {(!isCollapsible || isExpanded) && (
        <div className="border-2 border-gray-300 rounded-lg overflow-hidden">
          {/* Tabellen-Header */}
          <div className="bg-gray-200 px-4 py-3 flex items-center justify-between border-b-2 border-gray-300">
            <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">
              Arbeitspakete ({sortedWPs.length})
            </span>
            {showAddButton && onAddWorkPackage && (
              <button
                onClick={() => onAddWorkPackage(projectId)}
          ...
          <div>
            {sortedWPs.map((wp, idx) => renderWorkPackageRow(wp, idx === sortedWPs.length - 1))}
          </div>
          
          {/* Summen-Footer */}
          <div className="bg-gray-200 px-4 py-3 flex justify-between items-center border-t-2 border-gray-300">
            <span className="text-sm font-bold text-gray-700">Gesamt</span>
            <div className="text-right min-w-[80px]">
              <span className="text-sm font-bold" style={{ color: colors.primary }}>
                {totalPM.toFixed(2)} PM
              </span>
              <div className="text-xs text-gray-500">
                = {pmToHours(totalPM)} h
              </div>
