...
VERSION: 4.0 - 05.01.2026
FIXES:
- XFA mit Zeilenumbrüchen innerhalb Tags wird korrekt geparst
- Alle MA-Zuordnungen pro AP werden erfasst
...
def normalize_xfa(xfa_text: str) -> str:
    """
    Normalisiert XFA-Text: Entfernt alle Zeilenumbrüche
    Dies ist nötig weil ZIM-PDFs Zeilenumbrüche innerhalb von Tags haben
    """
    return xfa_text.replace('\n', '')


def parse_ap_nummer(lfd: str) -> tuple:
    """
    Parst AP-Nummern wie "1", "1.1", "2", "2.1" etc.
    Gibt (haupt_nr, unter_nr) zurück
    """
    if not lfd:
        return (0, 0)
    lfd = lfd.strip()
...
def parse_zim_pdf(pdf_path: str) -> dict:
    ...
    # XFA normalisieren (Zeilenumbrüche in Tags entfernen)
    xfa_norm = normalize_xfa(xfa_text)
    ...
    # === ARBEITSPAKETE === (KOMPLETT NEU MIT NORMALISIERTEM XFA!)
    arbeitspakete = []
    ap_temp = {}  # Key = AP-Code (z.B. "1.1"), Value = AP-Daten
    
    # Pattern für Zeile2 Blöcke im normalisierten XFA
    # Erlaubt sowohl <tag>text</tag> als auch <tag/> (leere Tags)
    ...
    for match in re.finditer(zeile2_pattern, xfa_norm):
        ...
        # Skip leere Zeilen (Überschriften ohne MA-Nr)
        if not ma_nr or not pm:
            # Aber merke den AP-Namen falls vorhanden
            if lfd and ap:
                current_ap_code = lfd
                current_ap_name = ap
            continue
        
        # Wenn neuer AP-Code mit Namen -> neues AP
        if lfd and ap:
            current_ap_code = lfd
            current_ap_name = ap
        
        # Wenn lfd ohne ap -> nutze gespeicherten Namen
        if lfd and not ap and current_ap_code and lfd == current_ap_code:
            ap = current_ap_name
        
        # AP erstellen falls nicht vorhanden
        if lfd and lfd not in ap_temp:
            ...
            ap_temp[lfd] = {
                ...
                'start_monat': None,  # Wird später gesetzt
                'ende_monat': None,
                'von_datum': von,
                'bis_datum': bis,
                'zuordnungen': []
