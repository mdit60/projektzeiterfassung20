...
function BeratePageContent() {
  ...
  // Auto-open ZA-Panel wenn ?panel=za in URL
  useEffect(() => {
    if (searchParams.get('panel') === 'za' && !loading && projects.length > 0) {
      const firstProjectId = projects[0].id;
      setZAProjectId(firstProjectId);
      setShowZAPanel(true);
      openZAPanel(firstProjectId);
      // Sanft zur ZA-Kachel scrollen
      setTimeout(() => {
        const el = document.getElementById('za-kachel');
