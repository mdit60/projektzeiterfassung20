...
//          - Portal-Rolle wird korrekt ermittelt (nicht hardcoded client_admin)
//          - employee: Redirect zur Zeiterfassung
//          - project_leader: Sieht nur zugeordnete Projekte/MA
// Fix v7.3.88-4: 
...
export default function BerichtePage() {
  ...
  const [portalRole, setPortalRole] = useState<'client_admin' | 'project_leader' | 'employee'>('employee');
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        let userPortalRole: 'client_admin' | 'project_leader' | 'employee' = 'employee';
        ...
        if (profile.role === 'client_admin') {
          ...
          if (authUser) {
            ...
            if (empRecord?.portal_role) {
              userPortalRole = empRecord.portal_role as typeof userPortalRole;
            }
          }
        }
        ...
        // employee hat keinen Zugang zu Berichten -> Redirect
        if (userPortalRole === 'employee') {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PortalHeader 
        ...
        userRole={userRole}
      />
      <PortalNav portal="firma" userRole={userRole} portalRole={portalRole} />
