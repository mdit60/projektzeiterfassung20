...
// Version: 7.3.67
//
// FIX: Projekte aus v7_work_package_assignments laden (Single Source of Truth)
//      statt aus v7_project_assignments
//
...
function ZeiterfassungInner() {
  ...
  const loadProjectLeaderData = async (companyId: string, employeeId: string, userId: string) => {
    // Projekte laden, denen der Projektleiter zugeordnet ist - aus v7_work_package_assignments
    const { data: wpAssignments } = await supabase
      .from('v7_work_package_assignments')
      .select(`
        work_package_id,
        v7_work_packages!inner (
          project_id,
    ...
    // Projekte extrahieren und deduplizieren
    const projectsMap = new Map<string, Project>();
    const projectIds: string[] = [];

    wpAssignments?.forEach(a => {
      const wp = a.v7_work_packages as any;
      if (wp?.v7_projects) {
        const proj = wp.v7_projects as Project;
    ...
    // MA dieser Projekte laden (alle die Work Package Assignments in diesen Projekten haben)
    if (projectIds.length > 0) {
      // Alle Work Packages dieser Projekte finden
      ...
      if (wpIds.length > 0) {
        // Alle MA mit Zuordnungen zu diesen Work Packages
        const { data: allWpAssignments } = await supabase
          .from('v7_work_package_assignments')
          .select('employee_id')
          .in('work_package_id', wpIds)
          .eq('is_active', true);

        const employeeIds = [...new Set(allWpAssignments?.map(a => a.employee_id) || [])];
  ...
  const loadEmployeeData = async (companyId: string, employeeId: string) => {
    ...
    // Nur zugeordnete Projekte laden - aus v7_work_package_assignments (Single Source of Truth)
    const { data: wpAssignments } = await supabase
      .from('v7_work_package_assignments')
      .select(`
        work_package_id,
