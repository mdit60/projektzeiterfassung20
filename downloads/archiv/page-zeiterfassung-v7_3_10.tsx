...
// VERSION: v7.3.10 (SW-Release V7.3)
// DATUM: 08. Januar 2026
// BESCHREIBUNG: Zeiterfassung - Layout optimiert für Druck auf eine A4-Seite
// ÄNDERUNGEN v7.3.10:
//   - Header: 2x3 Layout (Zuwendungsempfänger|Stundennachweis, Vorhabenthema|FKZ, Monat|Mitarbeiter)
//   - Unterschriften: senkrechte Trennlinie, Datum = letzter Arbeitstag (editierbar)
//   - "Zeitausgleich" → "sonstige Arbeiten"
//   - Datenlogik unverändert von v7.3.8
...
export default function ZeiterfassungPage() {
  ...
  const handleKeyDown = (
    ...
    // Hilfsfunktion: Nächstes editierbares Feld finden
    const isEditable = (r: number, d: number, type: 'ap' | 'nonbillable'): boolean => {
      if (isWeekend(selectedYear, selectedMonth, d) || isHoliday(selectedYear, selectedMonth, d)) return false;
      if (type === 'ap') {
        return apRows[r]?.workPackageId !== null;
      }
      return true; // nonbillable ist immer editierbar wenn kein WE/Feiertag
    };

    // Fokus auf Feld setzen
    const focusCell = (r: number, d: number, type: 'ap' | 'nonbillable') => {
      const selector = type === 'ap' 
        ? `input[data-row="${r}"][data-day="${d}"][data-type="ap"]`
        : `input[data-row="0"][data-day="${d}"][data-type="nonbillable"]`;
    ...
    // Nächstes freies Feld rechts finden (für Enter)
    const findNextFreeRight = (startRow: number, startDay: number): { row: number; day: number; type: 'ap' | 'nonbillable' } | null => {
      // Erst in der gleichen Zeile nach rechts
      for (let d = startDay + 1; d <= daysInMonth; d++) {
        if (isEditable(startRow, d, 'ap') && !apRows[startRow]?.entries[d]?.value) {
          return { row: startRow, day: d, type: 'ap' };
        }
      }
      // Dann in den nächsten Zeilen
      for (let r = startRow + 1; r < totalApRows; r++) {
        for (let d = 1; d <= daysInMonth; d++) {
          if (isEditable(r, d, 'ap') && !apRows[r]?.entries[d]?.value) {
            return { row: r, day: d, type: 'ap' };
          }
        }
      }
      // Dann in nonbillable
      for (let d = 1; d <= daysInMonth; d++) {
        if (isEditable(0, d, 'nonbillable') && !nonBillableEntries[d]?.value) {
          return { row: 0, day: d, type: 'nonbillable' };
        }
      }
      return null;
    };
    ...
    switch (e.key) {
      case 'ArrowRight':
        e.preventDefault();
        // Nächster Tag in gleicher Zeile
        for (let d = day + 1; d <= daysInMonth; d++) {
          if (isEditable(rowIndex, d, rowType)) {
      ...
      case 'ArrowLeft':
        e.preventDefault();
        // Vorheriger Tag in gleicher Zeile
      ...
      case 'ArrowDown':
        e.preventDefault();
        if (rowType === 'ap') {
          // Nächste AP-Zeile am gleichen Tag
          for (let r = rowIndex + 1; r < totalApRows; r++) {
            if (isEditable(r, day, 'ap')) {
              focusCell(r, day, 'ap');
              return;
            }
          }
          // Dann nonbillable
          if (isEditable(0, day, 'nonbillable')) {
      ...
      case 'ArrowUp':
        ...
        if (rowType === 'nonbillable') {
          // Zur letzten AP-Zeile am gleichen Tag
      ...
      case 'Enter':
        e.preventDefault();
        // Nächstes freies Feld rechts
        const next = findNextFreeRight(rowIndex, day);
        if (next) {
          focusCell(next.row, next.day, next.type);
        }
        break;
    }
  };
