...
export default function BerichtePage() {
  ...
  // Aktuell ausgewaehltes Projekt fuer Stundennachweis-Header
  const matrixProject = projects.find(p => p.id === (matrixProjectId || projects[0]?.id)) || projects[0] || null;
              ...
              <div className="mt-6 border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    {matrixProject && projects.length === 1 && (
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-gray-800">
                          {matrixProject.short_name || matrixProject.name}
                        </span>
                        {matrixProject.funding_reference && (
                          <span className="text-xs text-gray-500 font-mono bg-gray-100 px-2 py-0.5 rounded">
                            {matrixProject.funding_reference}
                          </span>
                        )}
                        {matrixProject.funding_format && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 font-medium">
                            {getFundingLabel(matrixProject.funding_format)}
                          </span>
                        )}
                      </div>
                    )}
