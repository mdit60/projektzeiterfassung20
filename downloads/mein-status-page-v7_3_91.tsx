...
//
// Zeigt dem eingeloggten Mitarbeiter eine Uebersicht seiner Projekte
// mit Ampel-Status pro Monat:
//   - Gruen:  Stunden vollstaendig erfasst
//   - Orange: Teilweise erfasst (mindestens 1 Stunde)
//   - Rot:    Keine Stunden erfasst
//   - Grau:   Monat liegt in der Zukunft
//   - Weiss:  Monat liegt ausserhalb des Projektzeitraums
//
// Klick auf einen Monat navigiert zur Zeiterfassung mit Parametern.
// Sichtbar fuer alle Rollen: client_admin, project_leader, employee.
//
// Architektur:
    ...
    return relevantProjects.map((project) => {
      ...
      for (let y = startYear; y <= endYear; y++) {
        ...
        for (let m = mStart; m <= mEnd; m++) {
          ...
          if (isFuture) {
            status = 'future';
          } else if (hoursRecorded > 0 && daysRecorded >= workingDays * 0.8) {
            // Mindestens 80% der Arbeitstage erfasst = vollstaendig
            status = 'complete';
            completeCount++;
        ...
        {hasProjects && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white rounded-lg shadow p-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500">Teilweise erfasst</p>
                              ...
                              {yearMonths.map((md) => {
                                ...
                                switch (md.status) {
                                  ...
                                  case 'partial':
                                    ...
                                    title = `${getMonthNameFull(md.month)} ${md.year}: Teilweise (${md.hoursRecorded.toFixed(1)}h an ${md.daysRecorded} von ${md.workingDays} Tagen)`;
                        ...
                        <span>
                          ...
                          {ps.partialMonths > 0 && (
                            <span className="text-orange-600">
                              {' '}| {ps.partialMonths} {ps.partialMonths === 1 ? 'Monat' : 'Monate'} unvollstaendig
                            </span>
                          )}
                        </span>
                      </div>
                  ...
                  {ps.missingMonths === 0 && ps.partialMonths > 0 && (
                    <div className="px-6 py-3 bg-orange-50 border-t border-orange-100 rounded-b-lg">
                      <div className="flex items-center gap-2 text-sm text-orange-700">
                        ...
                        <span>
                          {ps.partialMonths === 1
                            ? '1 Monat unvollstaendig'
                            : `${ps.partialMonths} Monate unvollstaendig`}
                        </span>
                      </div>
                    </div>
                  )}
          ...
          <div className="mt-8 bg-white rounded-lg shadow p-5">
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">Teilweise erfasst</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-gray-400 mt-3">
              ...
              Ein Monat gilt als vollstaendig, wenn mindestens 80% der Arbeitstage erfasst sind.
            </p>
          </div>
        )}
