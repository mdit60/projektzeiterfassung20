...
// - Login erstellen (Erkennt bereits registrierte Benutzer)
// - Passwort zuruecksetzen (NEU v7.3.91-1)
//
// AENDERUNGEN v7.3.91-1:
// - NEU: Passwort zuruecksetzen fuer MA mit bestehendem Login
//   Button mit Schluessel-Icon in Aktionsspalte (nur Berater-Portal)
//   Modal mit Passwort-Eingabe, nutzt Admin-API /api/v7/reset-password
...
export default function EmployeeManagement({
  ...
  // State - Passwort zuruecksetzen Modal (NEU v7.3.91-1)
  ...
  // NEU v7.3.91-1: Passwort zuruecksetzen Funktionen
  ...
  const handleResetPassword = async () => {
    if (!resetPwEmployee || !resetPwEmployee.user_id) return;
  ...
  return (
    <div className="space-y-6">
                    ...
                    {canEdit && (
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          {/* NEU v7.3.91-1: Passwort zuruecksetzen - nur wenn Login vorhanden und Berater-Portal */}
                          {emp.user_id && emp.is_active && portal === 'berater' && (
                            <button
      ...
      {/* ================================================================ */}
      {/* MODAL: Passwort zuruecksetzen (NEU v7.3.91-1) */}
      {/* ================================================================ */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
            <div className="flex justify-between items-center px-6 py-4 border-b">
              ...
              <button onClick={closeResetPwModal} className="text-gray-400 hover:text-gray-600">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              {resetPwSuccess ? (
                <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg text-sm flex items-start gap-2">
                  <Check size={18} className="mt-0.5 flex-shrink-0" />
                  <div>
                    <strong>Passwort erfolgreich zurueckgesetzt.</strong><br />
                ...
                <>
                  <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg text-sm">
                    Setzen Sie ein neues Passwort fuer den Mitarbeiter.
                    Das Passwort muss dem Mitarbeiter anschliessend mitgeteilt werden.
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Mitarbeiter</label>
                    <div className="px-3 py-2 bg-gray-100 rounded-lg text-gray-700">
                      {resetPwEmployee.display_name}
                    </div>
                  </div>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t bg-gray-50">
              ...
              {!resetPwSuccess && (
                <button
                  ...
                  disabled={resettingPw || !resetPwPassword}
                  className={`px-4 py-2 text-white rounded-lg disabled:opacity-50 bg-amber-600 hover:bg-amber-700`}
                >
                  {resettingPw ? 'Setze zurueck...' : 'Passwort zuruecksetzen'}
                </button>
              )}
            </div>
          </div>
