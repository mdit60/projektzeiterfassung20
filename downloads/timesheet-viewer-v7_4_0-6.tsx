...
// Datum: 17. April 2026
//
// v7.4.0-6: NEU: Offene Rueckfragen-Badge pro Firma im Accordion
...
function CompanyAccordion({
  company, projects, employees, assignments, timesheets, onNavigate, openNoteCount,
}: {
  company: ClientCompany;
  projects: Project[];
  employees: Employee[];
...
export default function TimesheetViewerPage() {
  ...
  const [openNotes,    setOpenNotes]   = useState<Record<string, number>>({}); // company_id -> Anzahl offener Notizen
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (proj && proj.length > 0) {
        ...
        // NEU v7.4.0-6: Offene Notizen pro Firma zaehlen
        ...
        if (notesRaw && notesRaw.length > 0) {
          const notesByCompany: Record<string, number> = {};
          notesRaw.forEach((n: { project_id: string }) => {
            const noteProject = (proj || []).find((p: Project) => p.id === n.project_id);
            if (noteProject) {
              const cid = noteProject.client_company_id;
              notesByCompany[cid] = (notesByCompany[cid] || 0) + 1;
            }
          });
              ...
              {companies.length === 0 ? (
                ...
                companies.map((company) => (
                  <CompanyAccordion
                    ...
                    openNoteCount={openNotes[company.id] || 0}
                  />
                ))
              )}
            </div>
