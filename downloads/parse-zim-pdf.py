...
def parse_zim_pdf(pdf_path: str) -> dict:
    ...
    for block in a62_blocks:
        ...
        if ma_id:
            ...
            for jahr_match in re.finditer(r'<cg_VMS_PK_iJahrZahl>(\d{4})</cg_VMS_PK_iJahrZahl>.*?<cg_VMS_PK_fPersMonat>([^<]+)</cg_VMS_PK_fPersMonat>', block, re.DOTALL):
                ...
                pm_str = jahr_match.group(2).replace(',', '.')
                try:
                    pm = float(pm_str)
                except:
                    pm = 0.0
    ...
    # Arbeitsplan-Zeilen
    for ap_match in re.finditer(r'<Zeile2><lfd>([^<]*)</lfd><ap>([^<]*)</ap><von>([^<]*)</von><bis>([^<]*)</bis><ma_nr>([^<]*)</ma_nr><pm>([^<]*)</pm></Zeile2>', xfa_text):
        lfd, ap, von, bis, ma_nr, pm = ap_match.groups()
        
        if lfd and ap:
            ap_nr = int(lfd) if lfd.isdigit() else len(ap_temp) + 1
            if ap_nr not in ap_temp:
                ap_temp[ap_nr] = {
                    'ap_nummer': ap_nr,
                    'ap_code': f'AP{ap_nr}',
                    'name': ap.strip(),
        ...
        if ma_nr and pm and ap_temp:
            current_ap = list(ap_temp.values())[-1]
            pm_str = pm.replace(',', '.')
            try:
                pm_val = float(pm_str)
            except:
                pm_val = 0.0
            if pm_val > 0:
                current_ap['zuordnungen'].append({
                    'ma_nr': int(ma_nr) if ma_nr.isdigit() else 0,
                    'pm': pm_val
                })
    ...
    if not ap_temp:
        ap_counter = 0
        for ap_match in re.finditer(r'<ap>([^<]+)<', xfa_text):
            ap_name = ap_match.group(1).strip()
            if any(ap['name'] == ap_name for ap in ap_temp.values()):
                continue
            ap_counter += 1
            ap_temp[ap_counter] = {
                'ap_nummer': ap_counter,
                'ap_code': f'AP{ap_counter}',
                'name': ap_name,
                'start_monat': None,
                'ende_monat': None,
                'zuordnungen': []
    ...
    for ap_nr in sorted(ap_temp.keys()):
        ap_data = ap_temp[ap_nr]
    ...
    print(f"Arbeitspakete: {len(arbeitspakete)} gefunden")
