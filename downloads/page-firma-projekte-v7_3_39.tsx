...
// VERSION: v7.3.39 (SW-Release V7.3)
...
// ÄNDERUNG v7.3.39: AP-Sortierung korrigiert für hierarchische Nummern (1.1, 1.2, 2, etc.)
// ÄNDERUNG v7.3.39: AP-Badge zeigt vollständigen ap_code (nicht nur Hauptnummer)
...
export default function FirmaProjektePage() {
  ...
  const handleSaveWP = async () => {
    ...
    try {
      ...
      const apCodeTrimmed = wpFormData.ap_code.trim();
      const apCodeMatch = apCodeTrimmed.match(/AP?(\d+)(?:\.(\d+))?/i);
  ...
  const sortWorkPackages = (wps: WorkPackage[]): WorkPackage[] => {
    return [...wps].sort((a, b) => {
      // Primär: Nach ap_number und ap_sub_number sortieren (aus DB)
      ...
      <footer className="bg-white border-t mt-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <p className="text-center text-sm text-gray-500">
            Projektzeiterfassung v7.3.26 Â· Firmen-Portal Â· Â© {new Date().getFullYear()}
          </p>
        </div>
      </footer>
