...
// Datum: 28. Maerz 2026
//
// v7.4.4-19: BUG FIX - Projektfortschritt-Kachel war als "Demnaechst" deployed
//            (Datei im Projektverzeichnis war noch v7.4.4-14, nicht -18)
//            - Fortschritt-Kachel jetzt korrekt aktiv (Button mit showFortschritt)
//            - Trigger-Zeile am Ende entfernt
//            - ProjektFortschrittPanel korrekt eingebunden
// v7.4.4-18: Stundennachweis-Matrix Completion-Flag aus v7_timesheet_completions
// v7.4.4-14: Kachel "Zahlungsanforderung" umbenannt, Inline-Panel
// v7.4.3-17: Echter XLSX Multi-Sheet Export
...
export default function BerichtePage() {
  ...
  const projectStats: ProjectStats[] = useMemo(() => {
    return projects.map(project => {
      ...
      const projectTimesheets = timesheets.filter(t =>
        t.project_id === project.id && t.is_billable === true
      );
  ...
  const employeeTimesheetStatus: EmployeeTimesheetStatus[] = useMemo(() => {
    ...
    return employeesInProjects.map(employee => {
      ...
      } else if (progressPercent > timeProgress + 10) {
  ...
  const matrixData = useMemo(() => {
    const projectId = matrixProjectId || projects[0]?.id;
    ...
    while (current <= endMonth) {
      const labels = ['Jan', 'Feb', 'Mrz', 'Apr', 'Mai', 'Jun',
    ...
    const projectWPIds = workPackages
      .filter(wp => wp.project_id === projectId)
      .map(wp => wp.id);
    ...
    const matrixEmployees = employees.filter(e => assignedEmpIds.includes(e.id));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {projectStats.length === 0 ? (
                  ...
                  projectStats.map(ps => {
                    const progressCapped = Math.min(100, ps.progressPercent);
                    const barColor = ps.status === 'critical' ? 'bg-red-500'
                    ...
                    return (
                      <tr key={ps.project.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          ...
                          {ps.project.short_name && (
                            <div className="text-xs text-gray-500">{ps.project.short_name}</div>
                          )}
                        ...
                        <td className="px-6 py-4 text-sm text-gray-500">
                          <div className="flex items-center gap-1">
                            ...
                            {ps.project.start_date
                              ? new Date(ps.project.start_date).toLocaleDateString('de-DE', { month: '2-digit', year: 'numeric' })
                              : '-'}
                            {' – '}
                            {ps.project.end_date
                              ? new Date(ps.project.end_date).toLocaleDateString('de-DE', { month: '2-digit', year: 'numeric' })
                              : '-'}
                          </div>
                          {ps.timeProgressPercent > 0 && (
                            <div className="text-xs text-gray-400 mt-0.5">
                              Zeitlich: {Math.round(ps.timeProgressPercent)}% verstrichen
                            </div>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right font-medium tabular-nums">
                          <span className={ps.status === 'critical' ? 'text-red-600' : 'text-gray-900'}>
                            {formatPM(ps.actualPM)}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="flex-1 bg-gray-200 rounded-full h-2.5" style={{ minWidth: '120px' }}>
                              <div
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex flex-col">
                ...
                {/* Aufklappbares Panel Personalkosten */}
                {showPKPanel && (
      ...
      <footer className="text-center py-4 text-sm text-gray-500 mt-8">
        Projektzeiterfassung v7.4.4-19 · {company?.name || 'Firmen-Portal'} · 2026
      </footer>
    </div>
  );
}
