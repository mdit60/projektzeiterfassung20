...
// V7 Import-Seite - DIREKTER PDF-UPLOAD
// VERSION: v7.2.4 - 06. Januar 2026
// FEATURES:
// - DIREKTER PDF-Upload über ZIM Parser Microservice
// - Unterstützt auch JSON-Upload als Fallback
// - Automatischer Reimport (alte Daten werden überschrieben)
// - Arbeitspakete + AP-Zuordnungen werden importiert
...
export default function V7ImportPage() {
  ...
  const handlePdfUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // PDF an externen ZIM Parser Service senden
      ...
      setState({ 
        loading: false, 
        error: '', 
        success: `PDF "${file.name}" erfolgreich analysiert! (${result.data.statistik?.anzahl_mitarbeiter || 0} MA, ${result.data.statistik?.anzahl_arbeitspakete || 0} AP)` 
      })
    } catch (err) {
      ...
      setState({ 
        loading: false, 
        error: `PDF-Analyse fehlgeschlagen: ${error.message}. Stelle sicher, dass es sich um ein ausgefülltes ZIM-Antragsformular (XFA-PDF) handelt.`, 
        success: '' 
      })
    }
  }, [])
            ...
            {!importResult && (
              <div className="bg-white rounded-lg shadow-sm border p-6">
                <h3 className="font-medium text-gray-900 mb-4 flex items-center">
                  ...
                  ZIM-Antrag PDF hochladen
                </h3>
                <p className="text-sm text-gray-500 mb-4">
                  Lade das <strong>Original-PDF</strong> des ausgefüllten ZIM-Antrags hoch (XFA-Format). 
                ...
                <div className="mt-6 pt-6 border-t">
                  <details className="text-sm">
                    <summary className="text-gray-500 cursor-pointer hover:text-gray-700">
                      ▶ Alternative: Vorbereitete JSON-Datei hochladen
                    </summary>
                    <div className="mt-3">
                      <input
                        ref={jsonInputRef}
