...
// Förderberatung Import-Seite
// VERSION: v7.1.0
// DATUM: 02. Januar 2026
// Basiert auf page-v7-import-FINAL-v2.tsx
'use client'
...
export default function V7ImportPage() {
  ...
  const handleImportToDatabase = useCallback(async () => {
    ...
    try {
      ...
      // 1. Firma prÃ¼fen/anlegen (v7_client_companies)
      ...
      // 2. Projekt prÃ¼fen/anlegen (v7_projects)
      ...
      if (existingProject) {
        ...
        const { data: newProject, error: projectError } = await supabase
          ...
          .insert({
            ...
            notes: `FÃ¶rderquote: ${parsedData.projekt.foerderquote}%, Gesamtkosten: ${parsedData.projekt.gesamtkosten}â‚¬, Zuwendung: ${parsedData.projekt.zuwendung}â‚¬, PM: ${parsedData.projekt.gesamt_pm}`,
            is_active: true,
          })
          .select('id')
          .single()
        ...
        if (existingEmployee) {
          ...
          const { data: newEmployee, error: employeeError } = await supabase
            ...
            .insert({
              ...
              notes: `Qualifikationsgruppe: ${ma.qualifikation_gruppe}, Jahresbrutto: ${ma.jahresbrutto}â‚¬, Stundensatz: ${ma.stundensatz}â‚¬, Teilzeitfaktor: ${ma.teilzeitfaktor}`,
              is_active: true,
            })
            .select('id')
            .single()
          ...
          if (employeeError) {
            console.error('Mitarbeiter-Fehler:', employeeError, 'fÃ¼r', displayName)
            continue
          }
      ...
      setState({
        loading: false,
        error: '',
        success: `âœ… Import erfolgreich! Firma "${parsedData.antragsteller.firma}", Projekt "${parsedData.projekt.kurzname || parsedData.projekt.name}", ${importedEmployees} neue Mitarbeiter angelegt, ${assignedEmployees} Projektzuordnungen erstellt.`
      })
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-6">
          <nav className="flex space-x-8">
            {[
              { id: 'zim-pdf', label: 'ðŸ“„ ZIM-Projektantrag', desc: 'PDF Import' },
              { id: 'stundennachweis', label: 'ðŸ“Š Stundennachweis', desc: 'Excel Import' },
              { id: 'manuell', label: 'âœï¸ Manuell', desc: 'Direkteingabe' },
            ].map((tab) => (
              <button
                key={tab.id}
        ...
        {activeTab === 'zim-pdf' && (
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <h3 className="font-medium text-blue-900 mb-2">ðŸ“‹ ZIM-FÃ¶rderantrag importieren</h3>
              <p className="text-sm text-blue-700 mb-3">
                Lade einen ausgefÃ¼llten ZIM-FÃ¶rderantrag (PDF) hoch. Die Daten werden automatisch extrahiert.
              </p>
              <div className="text-sm text-blue-600 space-y-1">
                <p><strong>UnterstÃ¼tzt:</strong> Einzelprojekt, Kooperationsprojekt, DurchfÃ¼hrbarkeitsstudie, Innovationsnetzwerk</p>
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
              ...
              <p className="text-sm text-gray-500 mb-4">
                WÃ¤hle einen ausgefÃ¼llten ZIM-FÃ¶rderantrag aus. Die Analyse erfolgt automatisch.
              </p>
              ...
              >
                {state.loading ? (
                  ...
                  'ðŸ“„ PDF-Datei auswÃ¤hlen'
                )}
              </button>
              
              {selectedFile && !state.loading && (
                <p className="mt-3 text-sm text-gray-600 text-center">
                  AusgewÃ¤hlt: <strong>{selectedFile.name}</strong>
                </p>
              )}
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4 text-sm bg-gray-50 p-4 rounded-lg">
                      ...
                      <div>
                        <span className="text-gray-500 block">FÃ¶rderquote:</span>
