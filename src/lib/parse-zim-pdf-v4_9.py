...
def extract_mitarbeiter_liste(xfa_text: str, format_type: str) -> list:
    ...
    if format_type == 'standard_zim':
        ...
        # BioInk-Format: <name>Tippl</name>, <vname>Mario</vname>, <quali>Dipl.-Ing.</quali>
        # Diese sind unter <MitarbeiterSeite><Page11><a71> zu finden
        
        # Versuche zuerst die lowercase Tags (BioInk-Format)
        nachnamen = extract_all_values_flexible('name', xfa_text)
        vornamen = extract_all_values_flexible('vname', xfa_text)
        qualifikationen = extract_all_values_flexible('quali', xfa_text)
        lfd_nummern = extract_all_values_flexible('lfd', xfa_text)
        
        # Filtere nur echte Namen (nicht Formulartexte)
        valid_nachnamen = []
        valid_vornamen = []
        valid_quali = []
        
        for i, name in enumerate(nachnamen):
            # Pruefe ob es ein echter Name ist (mind. 2 Zeichen, kein Formulartext)
            if name and len(name) >= 2:
                if name.lower() not in ['name', 'nachname', 'firma', 'antragsteller', 
                                        'berater', 'geschaeftsfuehrer', 'projektleiter']:
                    # Pruefe ob ein passender Vorname existiert
                    if i < len(vornamen) and vornamen[i] and len(vornamen[i]) >= 2:
                        if vornamen[i].lower() not in ['vorname', 'name', 'vname']:
                            valid_nachnamen.append(name)
                            valid_vornamen.append(vornamen[i])
                            valid_quali.append(qualifikationen[i] if i < len(qualifikationen) else '')
        
        # Erstelle Mitarbeiter-Liste (ohne Duplikate)
        seen_names = set()
        for i in range(len(valid_nachnamen)):
            nachname = valid_nachnamen[i]
            vorname = valid_vornamen[i]
            quali = valid_quali[i] if i < len(valid_quali) else ''
            
            # Duplikat-Check basierend auf Nachname + Vorname
            name_key = f"{nachname.lower()}_{vorname.lower()}"
            if name_key in seen_names:
                continue
            seen_names.add(name_key)
            
            mitarbeiter.append({
                'ma_nr': len(mitarbeiter) + 1,  # Fortlaufende Nummer
                'nachname': nachname,
                'vorname': vorname,
        ...
        # Fallback: Versuche auch Grossbuchstaben-Tags
        if not mitarbeiter:
            nachnamen = extract_all_values_flexible('Name', xfa_text)
...
def parse_legacy_euronorm(xfa_text: str, has_technical: bool = False) -> dict:
    ...
    # Kurzname: Wenn Projektname einen : enthält, nimm den Teil davor
    # Sonst: Verwende kurzfass falls vorhanden
    if projekt['name'] and ':' in projekt['name']:
        projekt['kurzname'] = projekt['name'].split(':')[0].strip()
    else:
        kurzfass = extract_value(r'<kurzfass[^>]*>([^<]+)', text)
        if kurzfass:
            # kurzfass ist oft die lange Beschreibung, kürze sie
            projekt['kurzname'] = kurzfass[:50] + '...' if len(kurzfass) > 50 else kurzfass
        elif projekt['name']:
            # Fallback: erste 30 Zeichen des Namens
            projekt['kurzname'] = projekt['name'][:30].strip()
...
def parse_standard_zim(xfa_text: str) -> dict:
    ...
    # Wenn kein Kurzname vorhanden, aus Projektname ableiten (Teil vor dem :)
    if not projekt['kurzname'] and projekt['name']:
        if ':' in projekt['name']:
            projekt['kurzname'] = projekt['name'].split(':')[0].strip()
        else:
            projekt['kurzname'] = projekt['name'][:30].strip()
...
def parse_zim_pdf(pdf_path: str, quiet: bool = False) -> dict:
    print(f"\n{'='*60}")
    print(f"ZIM PDF Parser v4.9")
    print(f"{'='*60}")
...
def main():
    if len(sys.argv) < 2:
        print("Verwendung: python3 parse-zim-pdf-v4_9.py <pfad-zur-pdf> [--json]")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    json_to_stdout = '--json' in sys.argv
    ...
    try:
        # Bei --json: Unterdrücke alle Print-Statements während des Parsens
        if json_to_stdout:
            import io
        ...
        # Bei --json: Stdout wiederherstellen und JSON ausgeben
        if json_to_stdout:
            sys.stdout = old_stdout
            print(json.dumps(data, ensure_ascii=False))
            sys.exit(0)
        
        # Sonst: In Datei speichern und Info ausgeben
