...
//
// FIX: UTF-8 Umlaute korrigiert (oe -> ö, ae -> ä, ue -> ü)
// FIX: Untertitel bei Kacheln entfernt (ZIM..., Forschungszulage...)
      ...
      try {
        ...
        if (profileData.consultant_company_id) {
          ...
          if (clientIds.length > 0) {
            // Schritt 2: Förderprojekte zählen (ZIM, BMBF_KMU - alles ausser FZUL)
            const { count: foerderCount } = await supabase
              .from('v7_projects')
              .select('*', { count: 'exact', head: true })
              .in('client_company_id', clientIds)
              .in('funding_format', ['ZIM', 'BMBF_KMU', 'OTHER'])
              .eq('is_active', true)

            projectCount = foerderCount || 0

            // Schritt 3: FZul-Projekte zählen
