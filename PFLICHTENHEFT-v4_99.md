...
**Status:** Session 50: Feature-Session. A-002 erledigt (NWM-Wording, TF v7.4.6-21). A-003 erledigt (AP-Quick-View Popup, TF v7.4.6-21). A-021 NEU+erledigt (NWM-Tagessperren + Cross-Projekt 9h-Grenze, TF v7.4.6-22, neue DB-Tabelle v7_nwm_blocked_periods). A-022 NEU+erledigt (Kapazitaetsplanung auf echte Arbeitstage x WAZ umgestellt, MA-Deep-Link mit Ruecksprung, KPT v7.4.8-17, FC v7.4.9-30). Bug-Fixes: Cockpit-Freischaltung fuer Berater (PortalHeader v7.3.95-13, Login v7.3.90-7), Header-Rollenanzeige im Firmen-Portal (portal_role aus v7_employees). A-020 als offen aufgenommen (Firmen-Deaktivierung im App-Paradigma).
...
| A-020 | Firmen-Deaktivierung fehlt im App-Paradigma. Muelleimer (is_active=false) existiert nur auf klassischer Foerderung-Seite. App-Pfad hat keine Firmen-Deaktivierung. | Martin | Session 49 | Offen | - | Loesungsansatz: handleDelete-Logik ins FirmaCockpit, mit Bestaetigungs-Dialog + Rueck-Navigation ins App-Cockpit. |
