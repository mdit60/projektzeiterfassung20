...
| EmployeeManagement.tsx | **7.3.95-18** | MA + modalOnly + onClose fuer Inline-Cockpit; A-024 E-Mail-Bestaetigungsfeld bei Neuanlage |
| MitarbeiterModal.tsx | **1.0.3** | Leichtes MA-Modal im Cockpit (new/edit/password); Gehaltsdaten (Anlage 6.1); A-024 E-Mail-Bestaetigung bei Neuanlage; A-031 Login-Erstellung im Passwort-Modus bei fehlendem Login (Formular + /api/v7/create-employee-login, ALREADY_REGISTERED abgefangen) |
...
| PortalHeader.tsx | **7.3.95-14** | Cockpit-Sync, portal_role fuer Firmen-Portal; dashboardHref im Berater-App-Modus -> App-Cockpit |
...
| TimesheetForm.tsx | **7.4.6-37** | A-021 NWM-Tagessperren + projektuebergreifende 9h-Tagesgrenze (otherProjectHours/calcCrossProjectTagSumme, gilt generell), A-002/A-003; A-026 U/K/S in AP-Zelle als Fehlzeit; v7.4.6-25..29 Doppelsave-Schutz + UX; A-030 Meine-Arbeitspakete-Popup; v7.4.6-31 Kurzarbeit + Rechtsklick-Auswahl, -32/-33 Fehlzeiten-Fixes, -34/-35 Wochenend-Erfassung nicht-foerderbar, -36 Rahmen-Fix (Tailwind 4); A-033 v7.4.6-37 MA-Auswahl aufs Projektteam (teamMemberIds/teamEmployees) |
| BerichtePage.tsx | **7.4.6-18** | Dashboard + .limit(10000); A-035 oberer Projekt-Dropdown setzt selectedReportProjectId UND matrixProjectId synchron (behebt Desync bei Mehr-Projekt-Firmen) |
| FirmaCockpit.tsx | **7.4.9-36** | Deep-Link ?editMA+?returnTo, Inline MA-Modal; A-020 Firmen-Deaktivierung; Prognose gesamt auf Plan gekappt (5.58); A-031 Schluessel-Icon-Tooltip + Hover-Farbe dynamisch je user_id (Login erstellen / Passwort zuruecksetzen), user_id in MA-Query; ASCII-Fix Firmen-Zaehler |
| ProjektFortschrittPanel.tsx | **7.4.5-24** | Zielerreichungs-Prognose, PDF-Export; Berechnung via projektfortschritt-utils (A-011); Prognose gesamt auf Plan gekappt + "Tempo ueber Plan"-Hinweis (5.58) |
| StundennachweisMatrix.tsx | **7.4.6-5** | Quelle: projectAssignments (Projektteam); A-029 Sammeldruck-Modus (Auswahl Spalten/Zeilen/Zellen -> StundennachweisSheet-PDF, nur Blaetter im Druck via #snw-print-root); A-032 MA-Zeilen auf aktives Projekt gefiltert (Mehr-Projekt-Firmen); A-035 Selbstheilungs-Guard auf activeProjectId (Fallback projects[0], wenn matrixProjectId nicht im uebergebenen projects-Array liegt) |
...
| lib/projektfortschritt-utils.ts | **7.4.9-2** | Shared Berechnungslogik: Monatsverlauf, Prognose, Szenarien; foerderMaximum=min(bewilligt, Plankosten x Satz); prognoseStundenAbrechenbar + tempoUeberPlan (5.58) |
| StundennachweisSheet.tsx | **1.0.0** | NEU: statisches Stundennachweis-Blatt (Anzeige/Druck), Layout 1:1 zum Einzeldruck; im Sammeldruck (A-029) verwendet |
...
| src/app/v7/berater/foerderung/firma/[id]/cockpit/stundennachweis/page.tsx | **7.4.9-6** | Matrix-Wrapper Cockpit; A-027 Navigation/returnUrl; A-032 assignment_start/end je MA+Projekt zusammengefuehrt |
...
| A-034 | Projektuebergreifende Abwesenheiten: Abwesenheit ist mitarbeiter-, nicht projektbezogen - einmal erfassen, in allen Projekten des MA wirksam (kein Doppeleintrag); Abwesenheitstag sperrt Arbeit projektuebergreifend. | Martin | Session 55 | Offen | - | Konzept KONZEPT-ABWESENHEITEN-ZENTRAL v1.1 abgenommen. Geklaert: ein Code/Tag, keine Teilabwesenheit, 9h-Deckel = Summe Arbeitsstunden ueber alle Projekte, Feiertage bleiben berechnet. Umsetzung als eigene DB-Session: neue Tabelle v7_employee_absences (partieller UNIQUE employee_id+work_date fuer is_active=true), Migration der Abwesenheits-Zeilen aus v7_timesheets (Backup + Konflikt-Klaerungsliste), dann Umstellung Lesepfade (Matrix/Sheet/Berichte) + Schreibpfad TimesheetForm + harte Cross-Projekt-Abwesenheitssperre. DEV->Verifikation->PROD. |
