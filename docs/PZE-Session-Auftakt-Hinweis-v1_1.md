...
## Ablage von Dokumenten (feste Regel)
Jedes Dokument, das Claude neu erstellt (Konzepte, Git-Sicherungen, Anleitungen, Checklisten, Hinweise etc.), legt Claude direkt im `downloads`-Ordner ab - nicht nur im Projekt. So wie bisher, nur dass Martin die Ablage nicht mehr selbst machen muss. Es gelten dabei weiterhin die Projektregeln: neue Datei mit eigener/erhoehter Build-Nummer, nichts ungefragt ueberschreiben oder loeschen; Verschieben und Loeschen bleibt ausschliesslich bei Martin.
