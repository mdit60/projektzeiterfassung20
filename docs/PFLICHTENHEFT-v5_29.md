...
**Status:** Session 72 (21.07.2026): **BERATER-/FIRMEN-COCKPIT - PROJEKT-KAESTCHEN DER MA-LISTE FUEHRT JETZT IN DIE STUNDENERFASSUNG - DEPLOYED V7.7.5 (reine UI/Navigation, keine Logik/DB).** GL-Wunsch (Katrin): In der MA-Liste des Firmen-Cockpits stehen unter jedem Mitarbeiter die Projekte, in denen er eingebunden ist. Diese Projekt-Kaestchen sind umgewidmet - der Klick fuehrt nicht mehr in die Team-Liste zum Bearbeiten des Mitglieds (das laeuft ohnehin ueber die Projektdaten), sondern direkt in die Stundenerfassung (Zeiterfassung), vorgefiltert nach genau diesem MA in genau diesem Projekt (?employee=&projekt=&returnUrl=). Bei mehreren Projekten je MA (z.B. Hendrik Linfert mit GRAVID + InGrav) bleibt jedes Projekt ein eigenes Kaestchen mit eigenem Link; Darstellung je Kaestchen: Projektname als Text + Uhr-Symbol. Route/Parameter 1:1 analog handleNavigateToZE der Stundennachweis-Matrix (cockpit-stundennachweis v7.4.9-7): Berater -> /v7/berater/foerderung/firma/[id]/zeiterfassung, Firma -> /v7/firma/zeiterfassung; year/month bewusst weggelassen (TimesheetForm belegt den aktuellen Monat vor), returnUrl fuehrt 'Zurueck' ins Firmen-Cockpit. handleTeamMemberClick durch handleStundenerfassungClick ersetzt (kein toter Code); ASCII-konform geprueft. Verhaltensvertrag FC-01..FC-07 unberuehrt (reine Navigation). **Deploy V7.7.5:** downloads -> src (FirmaCockpit-v7_4_9-36-4.tsx -> src/components/shared/FirmaCockpit.tsx), DEV-Test gruen (Zeiterfassung oeffnet mit korrektem MA + Projekt, im dev-Server-Log verifiziert), Commit auf v7-dev, merge v7-dev->main (--no-ff), push origin + cubintec, Vercel-Deploy auf pze.cubintec-hub.com. Details in GIT-SICHERUNG-v7_7_5-session72.md.
...
### 6a.1 Konzept
Projektweite Zusammenfassung ALLER ZAs fuer den Verwendungsnachweis (VN) gegenueber
VDI/VDE-IT. Kein eigenes PDF; feldgenaue Datenaufbereitung zum manuellen Uebertragen
ins offizielle VN-Formular + Druckbeleg (analog Paragraph 6.1). Zahlen sind ein
Read-Rollup ueber v7_zahlungsanforderungen + Timesheets (kein Doppeln). Details:
KONZEPT-VERWENDUNGSNACHWEIS-v1_0.md, Bauplan KONZEPT-VERWENDUNGSNACHWEIS-BAUPLAN-v1_0.md.
Stand: MVP DS De-minimis (Konzept/Bauplan fertig, Umsetzung ausstehend).

### 6a.2 Fuenf Varianten (Formular -> System)
| Variante | Formular_ID | funding_format / Merkmal |
|----------|-------------|--------------------------|
| Einzel-/Kooperationsprojekt | VDIVDE_VN_EP_Koop (3.02) | ZIM |
| DS nach AGVO | VDIVDE_VN_DS (3.03) | ZIM_DS, beihilfe_basis=agvo |
| DS De-minimis | VDIVDE_VN_DS_oAGVO (3.00) | ZIM_DS, beihilfe_basis=de_minimis |
| Netzwerk Phase 1 | VDIVDE_VN_PH1 (3.00) | ZIM_NETZWERK, Phase 1 |
| Netzwerk Phase 2 | VDIVDE_VN_PH2 (3.00) | ZIM_NETZWERK, Phase 2 |

Variante automatisch aus funding_format + beihilfe_basis + Phase. MVP nur DS De-minimis.

### 6a.3 Modus
Zwischennachweis = nur Sachbericht (KEINE Kosten-Zahlen). Schlussnachweis = Sachbericht
+ zahlenmaessiger Nachweis (Zusammenfassung aller ZA). Umschalter; MVP = Schlussnachweis.

### 6a.4 Kostenzeilen DS-oAGVO (Tabelle 6) und Reihenfolge-Mapping
Sechs feste Zeilen; die 30 % sind der Zuschlag fuer uebrige Kosten (NICHT der
Foerdersatz; DS-Foerdersatz = 70 %). Werte = Summe der ZA-Positionen ueber alle ZAs
im Berichtszeitraum. Die ZA-Reihenfolge bleibt unveraendert; der VN ordnet um:

| VN-Zeile | Wert (Summe ueber ZAs) | aus ZA-Zeile |
|----------|------------------------|--------------|
| 1 Personal technisch | pkT | ZA (1) |
| 2 Zuschlag technisch (30 %) | gkT = pkT x overhead_t/100 | ZA (2) |
| 3 Auftraege Dritte technisch | auftraege_dritte_t | ZA (5) |
| 4 Personal nichttechnisch | pkNT | ZA (3) |
| 5 Zuschlag nichttechnisch (30 %) | gkNT = pkNT x overhead_nt/100 | ZA (4) |
| 6 Auftraege Dritte nichttechnisch | auftraege_dritte_nt | ZA (6) |

T/NT-Trennung ueber work_package.is_technical; pkT/pkNT = Summe Stunden x
getHourlyRate (hourly_rate_approved-Vorrang). Formel 1:1 aus ZAPanel.

### 6a.5 Finanzierung (Tabelle 8)
bisher erhaltene Zuwendungen = Summe zahlungseingang_betrag der ZAs; Gesamt-Zuwendung
= Summe foerderbetrag_gesamt; noch zu erhaltende Zuwendung (Schlusszahlung) = Gesamt
minus bisher erhalten.

### 6a.6 Datenmodell
NEU v7_verwendungsnachweise (project_id, art zwischen|schluss, variante, formular_version,
berichtszeitraum_von/bis, sachbericht_ergebnis/_arbeitspakete/_auftraege/_kooperation/
_weiteres, status, created_at/updated_at). NEU v7_projects.beihilfe_basis
(de_minimis|agvo, Default de_minimis). RLS projektbezogen (v7_can_access_client),
analog ZA. SQL: SQL-MIGRATION-verwendungsnachweis-v1.sql (DEV+PROD).

### 6a.7 Einstiege
Eigene Kachel "Verwendungsnachweis" im Cockpit (Muster Cockpit|Fortschritt|
Stundennachweis) + Deeplink-Knopf im Kopf der ZA-Liste; beide oeffnen dieselbe
projektbezogene VN-Seite. Wrapper Berater
/v7/berater/foerderung/firma/[id]/verwendungsnachweis, Firma /v7/firma/verwendungsnachweis.

### 6a.8 Verhaltensregeln (VN-01 bis VN-08)
| Nr | Regel |
|----|-------|
| VN-01 | Variante automatisch aus funding_format + beihilfe_basis + Phase, nicht manuell. |
| VN-02 | Modus Zwischen/Schluss per Umschalter; Zwischen blendet den Zahlenteil aus. |
...
| VN-05 | Abgleich Ausschoepfung (Summe Foerderbetrag / bewilligte Summe) und offene Auszahlung (Foerderbetrag / Zahlungseingang). |
| VN-06 | Sachbericht-Texte projektweise gespeichert, ueber Sessions erhalten. |
