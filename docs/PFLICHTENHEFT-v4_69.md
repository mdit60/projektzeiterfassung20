...
**Status:** Session 24 abgeschlossen: Phase 1 Arbeitszeitgrenzen (Position-Dropdown + Teilzeit-Historie-Tabelle) live in PROD; kritische Lesson Learned zu v7-dev-first-Flow dokumentiert
...
## 7d. Arbeitszeitgrenzen (Phase 1) - NEU v7.4.7
...
Phase 1 liefert die **Datenbasis**; die Live-Validierung mit Ampeln
erfolgt in Phase 3.

### 7d.2 Was Phase 1 liefert
...
- `position_title` wird als Dropdown mit POSITION_OPTIONS geliefert
- Alt-Freitext-Werte fallen auf "Sonstige" + Freitext-Feld
- Gelber Hinweis bei Auswahl einer GF-Rolle ("50%-Regel fuer Projektzeit")
- State `sonstigeAktiv` sorgt fuer korrekte Dropdown-Anzeige bei Sonstige-Auswahl

### 7d.3 Datenmodell v7_employee_hours_history
...
### 7d.4 Naechste Phasen (geplant)

- **Phase 2:** UI-Dialog fuer Admins/Berater zum Anlegen zusaetzlicher
  History-Eintraege (Vollzeit→Teilzeit-Wechsel, Gueltig-ab-Datum)
- **Phase 3:** Live-Validierung der drei Grenzen mit Ampeln in TimesheetForm,
  StundennachweisMatrix und BerichtePage
- **Phase 4:** Sichtbarmachung in beiden Portalen (Compliance-Tabellen,
  Benachrichtigungen nicht noetig, UI-Warnung reicht)
...
|---------|-------|-------------|
| v4.69 | 21.04.2026 | Session 24: Phase 1 Arbeitszeitgrenzen (Teil 1) - neue Tabelle v7_employee_hours_history, Position-Dropdown mit Sonstige-Fallback + GF-Hinweis (EmployeeManagement v7.3.95-8), POSITION_OPTIONS+GF_POSITIONS+Konstanten in v7-types (v7.4.7-1), Dashboard-Scope-Bugfix offeneNotizen. Kritische Lesson Learned: v7-dev-first-Flow ist nicht verhandelbar. |
...
## 12f. KRITISCHE Arbeitsregel: v7-dev-first-Flow (NEU, Session 24)
...
In Session 24 wurde entdeckt, dass mehrere Aenderungen der Vorwochen (u.a. die
v7.3.95-7 mit `isEmpActive()`-Feature) direkt auf main entwickelt worden waren.
Die downloads/-Snapshots waren dadurch veraltet. Eine darauf aufsetzende
Phase-1-Aenderung hat die main-Version still ueberschrieben und musste per
Forward-Fix repariert werden (Commit `6975a1b`).
...
**Abhilfemassnahme seit Session 24:** Memory-Eintrag #27 dokumentiert die Regel
fuer alle zukuenftigen Chats/Sessions.

---

**Ende des Pflichtenhefts v4.69**
