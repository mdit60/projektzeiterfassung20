...
**Status:** Session 68: **FZul-/MULTIPROJEKT-MODUL END-TO-END FUNKTIONSFAEHIG IN PRODUKTION (V7.7.0).** Aus den in PZE erfassten Foerderprojekten ermittelt das Multiprojekt-Tool je MA die tagesgenau gebuchten Foerderstunden; die Differenz zur Tagesarbeitszeit = die fuer die Forschungszulage verfuegbaren Stunden (Abgrenzung Doppelfoerderung). Das spart den manuellen Abgleich abgerechneter Foerderprojekte je MA/Projekt. **Behoben/neu diese Session:** (1) **CRITICAL day_type-Enum-Filter** (multiprojekt-detail v7.4.8-14): der Import schloss Abwesenheiten mit deutschen Werten aus (urlaub/krank/sonderurlaub/feiertag), die Enum v7_day_type ist aber ENGLISCH (vacation/sick/special_leave/holiday, work, weekend, short_time) -> PostgREST-Enum-Cast-Fehler -> v7_timesheets-Query lieferte null (Fehler nicht abgefangen) -> gefoerdert=0 -> ALLE MA faelschlich in Gruppe B, Kalender zeigte ueberall volle 8h. Kernursache "Import=0". Filter auf englische Enum-Werte korrigiert (2 Stellen: loadVorhaben + handleImport). (2) **ZIM-Erkennung** (v7-types v7.4.9-3, additiv): 'ZIM' und 'ZIM_DS' in Typ-Union, Label-/Short-Maps und V7_PUBLIC_FUNDING_FORMATS ergaenzt - Alt-Projekte mit generischem funding_format 'ZIM' (in PROD der Regelfall, z.B. HEATS) wurden sonst nicht als gefoerdert erkannt. (3) **Multijahr-Kalender** (v7.4.8-15): gefoerderte Stunden werden je ANGEZEIGTEM Jahr live aus v7_timesheets berechnet (statt nur aus gespeicherten FZul-Zeilen des Vorhaben-Jahres) -> Jahres-Navigation zeigt fuer jedes Jahr die freie FZul-Kapazitaet; gefoerdert/verfuegbar damit immer aktuell. (4) **Export-Tab** (v7.4.8-16, Phase 3, pro MA eine Datei): erzeugt je MA die amtliche BSFZ-Excel fuers gewaehlte Jahr ueber die BESTEHENDE /api/export/fzul (v2.3, unveraendert) und laedt sie herunter; uebergeben werden die je Tag in Foerderprojekten gebuchten Stunden, die Vorlage schreibt daraus die max fuer FZul verfuegbaren Stunden je Tag (Tagesarbeitszeit - gebucht); Jahresarbeitszeit/FuE-Anteil/Hoechstgrenze rechnet die Vorlage selbst. Eine zunaechst begonnene API-Erweiterung (v2.4, direkte fue_hours) wurde als ueberfluessig verworfen - das Tool liefert an dieser Stelle bewusst nur die verfuegbare Kapazitaet; das Verteilen auf konkrete FZul-Vorhaben ist eine spaetere, separate Funktion. (5) **Vorhaben-Loeschen-Button** (multiprojekt-page v7.4.8-19): Papierkorb je Vorhaben mit Sicherheitsabfrage, loescht timesheets + vorhaben. **Deploy V7.7.0:** merge v7-dev->main (--no-ff), push origin + cubintec, Vercel-Deploy auf pze.cubintec-hub.com verifiziert; **KEIN Migrations-/Enum-Schritt** (v7_fzul_vorhaben/v7_fzul_timesheets + granulare RLS-Policies - Berater SELECT/INSERT/UPDATE/DELETE + system_admin ALL - existierten bereits in DEV UND PROD). In PROD verifiziert (Gruppe A zieht gefoerderte Stunden, Multijahr-Kalender, Export). **ERKENNTNIS:** Die Enum v7_funding_format divergiert zwischen DEV (ZIM_EINZEL/KOOP/NETZWERK, BMBF_KMU, FZUL, OTHER, ZIM, ZIM_DS) und PROD (ZIM, ZIM_DS, ZIM_KOOP, ZIM_NETZWERK, BMBF, BMBF_DS, EFRE, Horizon, Landesprogramm, FZul, Sonstige); real genutzt werden nur ZIM-Varianten (vom Fix abgedeckt) - Vereinheitlichung Richtung B als eigener Track offen. **OFFEN / NAECHSTES GROSSES THEMA:** Bereich 'Forschungszulage' (Tile fzul) - ausser den Firmen-Kacheln noch nichts umgesetzt; Kachel-Klick fuehrt auf /v7/berater/fzul/firma/<id> -> 404 (Route existiert nicht). Ferner: Enum-Vereinheitlichung DEV/PROD; Multijahr-Feinschliff (Tagesarbeitszeit je Jahr aus Teilzeit-Historie statt Vorhaben-Stichtag); spaeter/separat 'freie FZul-Zeit intelligent auf konkrete FZul-Vorhaben verteilen' (neue Funktion). Offen unveraendert: A-001, A-006, A-034-Restpunkt (RLS-Angleich DEV/PROD). Komponenten dieser Session: v7-types v7.4.9-3, multiprojekt-detail v7.4.8-16 (kumuliert -14/-15/-16), multiprojekt-page v7.4.8-19; api/export/fzul route v2.3 unveraendert. **NACHTRAG Session 68 (15.07., zweiter Deploy V7.7.0 - Code, keine DB): BEREICH FORSCHUNGSZULAGE GEKLAERT - die oben notierte Aussage "naechstes grosses Thema, ausser Firmen-Kacheln noch nichts" ist damit UEBERHOLT.** Recherche in den Planungsdokumenten ergab eine in Vergessenheit geratene Entscheidung: **KONZEPT-KAPAZITAETSPLANUNG v1.1 (23.04.2026)** hebt das FZul-Konzept ausdruecklich auf eine hoehere Ebene ("Die FZul-Auswertung ist eine Spezialisierung der allgemeinen Kapazitaetsplanung"; §6: "ein Exportweg aus der Kapazitaetsplanung - nicht mehr der primaere Einstieg"). Anlass war laut §1.1 eine Rueckmeldung von Katrin Kirchner (Berater wollen jederzeit freie Kapazitaet sehen, unabhaengig vom Zweck). **Ein eigenstaendiges FZul-Modul war seit April 2026 also gar nicht mehr vorgesehen**; die `fzul`-Seite (v7.3.1/v7.4.0, Jan/Feb 2026) stammt aus der Zeit davor und wurde nie zurueckgebaut - daher `coming_soon`, leerer href und Firmen-Kacheln auf die nie gebaute Route /v7/berater/fzul/firma/<id> (404). Einem separaten FZul-Modul war je nur zugedacht (KONZEPT-MULTIPROJEKT-FZUL v1.2 §8): "MAs ohne gefoerderte Projekte abbilden" (erledigt - Gruppe B) und die **BSFZ-Bescheinigungsbeantragung** (nicht gebaut) - nur Letzteres bleibt fachlich uebrig. **ENTSCHEIDUNG (Martin):** FZul-Tile fuer alle Berater ausgeblendet, nur noch fuer `system_admin` sichtbar als Werkbank zur Vorbereitung eines kuenftigen FZul-Moduls; der 404 dahinter bleibt bewusst. **BEFUND (Ursache, dass die Aenderung zunaechst nur halb griff):** Der FZul-Einstieg ist an VIER unabhaengigen, hartkodierten Stellen definiert, keine liest die Modul-Config - PortalNav (NAV_BERATER), berater/dashboard (eigenes kacheln-Array), AppNav (eigenes navItems), app/cockpit ("Kachel 4"). Widerspricht dem Shared-Components-Prinzip; zudem divergieren die Listen (nur 3 der 10 Module sind Beraterwerkzeuge, "Unternehmen"/"Administration" fehlen in der Config ganz; dieselbe Kachel als `foerderung` vs. `kunden`, Kapazitaetsplanung als `kapazitaet` vs. `multiprojekt`; Config-Label "Multiprojekt-Tool" vs. UI "Kapazitaetsplanung" = offene §8-Umbenennung; Icons als String vs. Komponente und inhaltlich uneins). **Umgesetzt:** PortalNav v7.4.4-26 (`isAdmin: true`), v7-module-config v7.3.90-8 (`roles: ['system_admin']`, Beschreibung/plannedRelease korrigiert), berater-dashboard-page v7.4.4-14, AppNav v1.0.2, berater-app-cockpit-page v1.0.8. **FIX Vorhaben-Modal (multiprojekt-page v7.4.8-19 -> v7.4.8-20):** Das Modal belegte die Firma IMMER mit companies[0] (alphabetisch erste), unabhaengig vom FIRMA-Dropdown -> Vorhaben konnten unbemerkt auf der falschen Firma landen. Neu: Prop defaultCompanyId (= selectedCompanyId); Formular wird beim OEFFNEN zurueckgesetzt und vorbelegt (noetig, da das Modal montiert bleibt und der State sonst ueberlebt; Effekt bewusst nur von isOpen abhaengig). Verifiziert: als system_admin ueberall sichtbar, gegen ein Berater-Konto (Katrin, PROD) nirgends mehr. **NEU im Backlog:** Berater-Einstiege in eine gemeinsame Liste zusammenfuehren - Empfehlung gemeinsam mit der §8-Umbenennung (dieselben Labels an denselben Stellen), ca. halbe bis dreiviertel Session; volle Konsolidierung auf v7-module-config ~1 Session, fasst aber die komplette Navigation an. **Matrix-Detail-Popup (KPT §5.2) ZURUECKGESTELLT:** faktisch erfuellt - der Hover-Tooltip zeigt bereits Monatskapazitaet, je Projekt geplant/verbucht und "Frei: X h (Y%)"; es fehlt nur ein klickbares Popup (Tooltip hat pointerEvents:'none') mit "+ FZul-Auswertung erstellen"; Nutzen gering, da ein Vorhaben heute ein FuE-Vorhaben mit Titel/Wirtschaftsjahr/mehreren MA ist und der Dialog fast derselbe waere. **RECHTE-BUG BEHOBEN (RLS, DEV+PROD):** Die Policies waren asymmetrisch - LESEN beraterfirmen-weit, aber BEARBEITEN/LOESCHEN ersteller-gebunden (created_by = auth.uid()). Ein Berater sah damit Vorhaben von Kollegen, konnte daran aber NICHTS tun: Loeschen, Import UND Kalender-Speichern schlugen fehl - und zwar **LAUTLOS**, weil PostgREST bei 0 betroffenen Zeilen KEINEN Fehler liefert (der Code prueft nur `error`). **LEHRE: Bei DELETE/UPDATE unter RLS immer `.select()` anhaengen und pruefen, ob Zeilen betroffen waren.** ZIELBILD (Vorgabe Martin): Berater liest+bearbeitet alle Vorhaben der eigenen Beraterfirma, loescht nur selbst angelegte; SysAdmin darf alles. Umgesetzt mit **SQL-MIGRATION-fzul-rls-bearbeiten-beraterfirma-v1.sql** (DEV+PROD): NUR die BEARBEITEN-Policies auf die Beraterfirmen-Logik gezogen (v7_fzul_vorhaben.UPDATE, v7_fzul_timesheets.INSERT/UPDATE); die DELETE-Policies bleiben bewusst ersteller-gebunden (waren also von Anfang an richtig), ebenso *_system_admin_all. Soll-Bild in beiden Umgebungen verifiziert. **ERSTELLER-ANZEIGE:** Die Vorhabenliste zeigt jetzt "angelegt von X" (bei zwei Beratern Beiwerk, bei groesseren Beratungen der eigentliche Nutzen: wen spreche ich an?). Noetig war **SQL-MIGRATION-beraterfirma-namen-v1.sql**: Die SELECT-Policy auf v7_user_profiles ist eng ((id = auth.uid())) und laesst sich NICHT erweitern - eine Policy auf v7_user_profiles, die selbst v7_user_profiles liest, erzeugt eine ENDLOSREKURSION (dokumentierte Falle). Loesung: SECURITY DEFINER-Funktion v7_beraterfirma_namen(), liest intern an der RLS vorbei und gibt NUR id + Anzeigename der Kollegen der eigenen Beraterfirma zurueck (keine E-Mail/Rolle); gleiches Muster wie v7_is_system_admin(). Die enge SELECT-Policy bleibt UNVERAENDERT (verifiziert). Verworfen: Profil-SELECT fuer Kollegen oeffnen (haette die ganze Profilzeile preisgegeben). **UX-ENTSCHEIDUNG (kein rot/gruener Papierkorb):** Erwogen und verworfen - Rot bedeutet konventionell "destruktive Aktion" (deshalb sind Loeschen-Buttons rot), nicht "verboten"; Gruen ist fuer Loeschen semantisch schief; Farbe als einzige Information ist nicht barrierefrei. Stattdessen ausgegrauter Papierkorb (text-gray-200, cursor-not-allowed) + Tooltip mit Namen; klickbar bleibt er, damit die erklaerende Meldung kommt. **FIX Vorhaben-Modal (v7.4.8-20):** belegte die Firma IMMER mit companies[0] (alphabetisch erste) statt der oben gewaehlten -> Vorhaben konnten unbemerkt auf der falschen Firma landen. Neu: Prop defaultCompanyId (= selectedCompanyId); Reset beim OEFFNEN (noetig, da das Modal montiert bleibt und der State sonst ueberlebt; Effekt bewusst nur von isOpen abhaengig). **Loeschen-Kette v7.4.8-21 bis -25:** -21 Vorab-Ersteller-Pruefung (sofortiger Hinweis OHNE vorherige Sicherheitsabfrage) + DELETE mit .select('id'); -22 Sprachkonvention; -23 ausgegrauter Papierkorb + zentrale darfVorhabenLoeschen() (Optik und Funktion koennen nicht auseinanderlaufen); -24 Ersteller-Anzeige via RPC; -25 title-Attribut (voller Name beim Hovern, da die Spalte kuerzt). **NEUE KONVENTION - SPRACHE (Vorgabe Martin, gilt projektweit):** In ALLEN UI-Texten, Meldungen, Beschreibungen, Dokumenten und Anleitungen durchgaengig nur EIN Genus - generisches Maskulinum (der User, Administrator, Berater, Anwender, Ersteller). Keine Doppelnennungen, keine Gender-Sonderzeichen. **BEFUND display_name-Konvention DEV vs. PROD:** PROD "Ditscherlein, Martin" (Nachname, Vorname), DEV "Martin Ditscherlein". DEV ist der Ausreisser - der Code schreibt die Konvention bereits korrekt (ConsultantManagement.tsx:296 -> last_name || ', ' || first_name; EmployeeManagement.tsx:839 analog fuer v7_employees), v7_employees folgt ihr durchgaengig ("Bohlmann, Jens"), und der BSFZ-Excel-Export verlaesst sich darauf (splittet empName am Komma). Konvention bleibt **"Nachname, Vorname"** (der Nachname bleibt bei der unvermeidlichen Kuerzung sichtbar). Nur DEV wird per SQL-DATENKORREKTUR-dev-display-name-konvention-v1.sql angeglichen (Preview+UPDATE+Kontrolle), KEIN Code-Fix. Relevant fuer kuenftige PROD->DEV-Syncs. Details in GIT-SICHERUNG-v7_7_0-session68-nachtrag.md.
