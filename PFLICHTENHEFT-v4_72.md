...
**Status:** Session 29 abgeschlossen: NWM-Tabellen in PROD migriert (Yacht Connect Jahr 1+2), 60 Timesheet-Duplikate bereinigt, KPT Frei-PM-Spalte ergaenzt, Git-Branches synchronisiert.
...
**#0 - Unique Constraint v7_timesheets (PROD)**

Verhindert kuenftige Duplikate. Constraint auf
(employee_id, project_id, work_date, work_package_id) -- NUR wenn eindeutig.
Achtung: legitime Mehrfachbuchungen auf verschiedene APs am gleichen Tag
sind erlaubt. Muss sorgfaeltig geplant werden.
...
### 3.12 Session 27-29 (23.-24. April 2026) - MPT/FZul, KPT, PROD-Migration

**Session 27 (23. April 2026):**
- Multiprojekt-Tool Phase 1+2: Uebersichtsseite + FZul-Jahreskalender
  (`berater-multiprojekt-page-v7.4.8-1` bis `-4`, `berater-multiprojekt-detail-v7.4.8-1` bis `-12`)
- FZul-Kalender: Excel-Navigation (Tab/Enter/Pfeile), Jahreswechsel, Farbgebung
- Dashboard-Kachel: "Kapazitaetsplanung" + "FuE-Vorhaben"
- Modul-Config: MPT aktiv
- DB-Migration: `v7_fzul_vorhaben`, `v7_fzul_kalender_eintraege` (DEV+PROD)

**Session 28 (23. April 2026):**
- KPT Phase 1: Kapazitaetsmatrix (MA x Monat, Ampelfarben, Tooltip, Jahreswechsel)
  (`berater-multiprojekt-page-v7.4.8-4`)
- NWM-Integration in KPT: liest aus `v7_nwm_ap_planung`
- NWM Foerderzeitraeume: `v7_nwm_foerderzeitraeume` + `v7_nwm_ap_planung` (DEV)
- Yacht Connect AP-Daten: Jahr 1 (19.45 PM) + Jahr 2 (12.622 PM) in DEV
- Arbeitsplan Jahres-Navigation NWM: `ProjectDetailPage-v7.4.4-54`, `WorkPackageTable-v7.4.3-11`
- PROD→DEV Datenkopie (9 Firmen, 32 MA, 3585 Timesheets)

**Session 29 (24. April 2026):**
- Git-Branches synchronisiert (v7-dev war hinter main)
- NWM-Tabellen in PROD migriert (Yacht Connect Jahr 1+2 vollstaendig)
- 60 Timesheet-Duplikate in PROD bereinigt (Import-Script-Mehrfachausfuehrung)
- FZul-Testprojekt in `v7_fzul_vorhaben` PROD geloescht
- KPT: Frei-PM-Spalte ergaenzt (`berater-multiprojekt-page-v7.4.8-5`)
- Lernpunkt: `v7_fzul_vorhaben` ist eigenstaendige Tabelle (Spalte: `title`)
- Lernpunkt: `MIN(id)` auf UUID in PostgreSQL nicht moeglich -> `MIN(created_at)` verwenden
...
|---------|-------|-------------|
| v4.72 | 24.04.2026 | Session 29: NWM-Tabellen in PROD migriert (v7_nwm_foerderzeitraeume + v7_nwm_ap_planung). Yacht Connect Jahr 1 (35 Eintr./19.45 PM) + Jahr 2 (28 Eintr./12.622 PM) in PROD. 60 Timesheet-Duplikate bereinigt (Ursache: Import-Script mehrfach ausgefuehrt). FZul-Testprojekt in v7_fzul_vorhaben geloescht. KPT: Frei-PM-Spalte ergaenzt (berater-multiprojekt-page v7.4.8-5). Git-Branches v7-dev/main synchronisiert. |
